<template>
  ...
  <div v-show="isChannel" ref="AccountFrom" :model="account" :rules="rules" class="login-container">
    ...
    <div class="loginBox" style="margin-top: 56px" >
  ...
  export default {
    ...
    methods: {
      cityClick(value){ //城市选择
        window.location.href =`http://${value}.${this.$common.domainUrl}`;
      },
      handleLogin() {
        ...
        this.$http.post(url,params).then(res=>{
            ...
            if(data.code==="0000"){
              ...
              Cookies.set('access-user', JSON.stringify(data.result))
              setSession("userInfo",data.result);
              setLocalStorageUserInfo("enrolleeinfo",data.result);
              Cookies.set("enrolleeinfo",JSON.stringify(data.result));
              setSession("fle",true);
              setSession("update","0")
...
//              this.$queryFun.successAlert.call(this.data.error);
    ...
    created(){
      let href = window.location.href,str = href.substring(href.indexOf('//')+2,href.indexOf('.'));

      this.cityValue = str;
//      this.cityClick(this.cityValue)
...
//      this.$http.get(mapUrl).then(res1=>{
//        console.log(res1)
//        this.$http.get(cityUrl).then(res=>{
//          if(res.data.code === "0000"){
//            this.options.options1 =res.data.result;
//            this.options.options1.forEach((obj,index)=>{
//              if(obj.cityName === res1.data.city) {
//                this.cityValue = obj.domain;
//              }else if(obj.domain===str){
//                this.cityValue = obj.domain;
//              }
//            })
//          }else{
...
//        })
      ...
      if(getSession("KEY")[0]==="3"&&window.location.href.indexOf("http://localhost")===-1&&false){//上线用
        setTimeout(()=>{

...
const OrderList6 = resolve => require(['@/components/order/complete'], resolve);//完成工单
const askForHelp = resolve =>require(['@/components/order/askForHelp'],resolve);//问题求助
const unAudited = resolve =>require(['@/components/order/unAudited'],resolve);  //待审核
      ...
      // },
        ...
        },
        {
            ...
            children: [
                    ...
                    { path: '/order/list4',component: OrderList4,name: '工单报备',menuShow: isShowFunObj.isUserNavChild("sn-order-pending")},
                    ...
                    { path: '/order/list10', component: OrderList10, name: '线下结款工单', menuShow:true },
                    { path: '/order/unAudited', component: unAudited, name: '待审核', menuShow:true },
                    { path: '/order/list6', component: OrderList6, name: '完成工单', menuShow:true },
        ...
        //渠道 start
        {
          ...
          children: [
            ...
            { path: '/census/canalCensus', component: dayOrder, name: '全部工单', menuShow: true },
...
router.beforeEach((to, from, next) => { // 路由跳转前认证
    if (to.path.startsWith('/login')) {  // 判断是不是登录页面 是
        window.sessionStorage.removeItem('access-user');  // 是登录页面删除信息
       //Cookies.remove('access-user');
        next() //下一步
    } else { // 不是登录页面
        let user = JSON.parse(window.sessionStorage.getItem('access-user'))   //查看本地存储有没有信息
       //let users = Cookies.get('access-user');

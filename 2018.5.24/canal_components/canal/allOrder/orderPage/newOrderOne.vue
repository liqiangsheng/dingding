<template>
  ...
  <WorkOrderSubmission @isClose="isClose" v-if="chongzhiShow1"></WorkOrderSubmission>
    <LookAtMoreAlter @isClose="isClose" v-if="lookAtMoreShow"></LookAtMoreAlter>
  <div style="overflow-y: auto;padding: 20px;">
            ...
            <div class="timeSlot" v-show="timeBoxShow">
              ...
              <ul>
                <li id="hour" v-for="(item,index) in hour">
                  <el-button :disabled="item.bool" :class="{timeActive:timeIndex == index}" @click="timeClick(item,index)" type="info">{{item.time}}</el-button>
                </li>
              </ul>
    ...
    <div class="chanpin">
      ...
      <div id="history">
        <p>历史下单<br>（快捷选择）</p>
        <ul>
          <li v-for="(item,index) in historyData">{{item}}</li>
        </ul>
        <p class="more" @click="lookAtMore">查看更多工单</p>
        <el-button type="primary" style="position: absolute;left: 7%;bottom: 20px">确定添加</el-button>
      </div>
  ...
  export default {
    ...
    data() {
      return {
        ...
        historyData:["啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊阿","巴巴爸爸不不不不反倒是所所所所所所所所所所所所所所所所所所所所所不不不不不不不不不不不不不不","啛啛喳喳错错错错错错错错错错错错错错错错","啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊阿"],

export let mutations={//同步处理vueX
  repairOrderId(state,data){//订单详情的Id
    state.repairOrderId = data;
  },
  partsAdministrationObj(state,data){//配件管理
     state.partsAdministrationObj = data;
  },
  cityUrl(state,data){//城市登录的Url
   state.cityUrl = data;
  },
  orderDetailiD(state,data){//订单详情的Id
    state.orderDetailiD = data;
  },
  parseDetailiD(state,data){ //配件详情的Id
    state.parseDetailiD = data;
  },
  xiaoqudaoId(state,data){
      state.xiaoqudaoId =data;
  },
  weachat(state,data){
    state.weachat = data;
  },
  ...
  cityIdid1(state,data){
    state.cityIdid1 = data;
  },
  ...
  getMainChannel(state,data){
    state.mainChannel=data;
  },
  getClassify(state,data){
    state.classifyData=data;
  },
  orderDetailsState(state,data){ //订单状态
...
//  渠道 弹框数据状态
  ...
  spliceTabByPath(state,path){
    state.tabPathList.forEach((item,index)=>{
       if(path === item.path){
         state.tabPathList.splice(index,1);
       }
    })

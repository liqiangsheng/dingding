        ...
        <div class="dable_lsit" v-if="puductShow">
          ...
          <table id="table" cellSpacing="0px" cellpadding="0">
            ...
            <tr v-for="(item,index) in tableListData.list">
              ...
              <!--质保时间 （天）-->
              <td >
                {{item.warrantyDatetime==null?0:item.warrantyDatetime}}
              </td>
              <!--状态-->
              <td>
              ...
              <!--操作-->
              <td class="cursorSize">
                <span class="cursor" style="color: #20A0FF"  @click="edit('产品',item)">编辑</span>
                <span class="cursor" style="color: #20A0FF" @click="partsAdministration(item)">配件管理</span>
                <span class="cursor" style="color: #0C9073" @click="del('产品',item,1)" v-if="item.state==0">上架</span>
                <span class="cursor" style="color: #4A4A4A" @click="del('产品',item,2)" v-if="item.state==1">下架</span>
                <span class="cursor" style="color:#E65831 " @click="del('产品',item,3)">删除</span>
              </td>
            </tr>
            </tbody>
          </table>
  ...
  export default {
    ...
    data() {
      return {
        ...
        stateType:[{id:"0,1",name:"--请选择--"},{id:"0",name:"未上架"},{id:"1",name:"已上架"}], //状态
        stateName:"",//状态
        stateId:"0,1",//状态ID
        tabListName:["序号","商品编号","产品名称","服务费类型","上门费 （元）","收上门费","服务费 （元）","质保时间 （天）","状态","操作"]
      }
    },
...
//    },
    ...
    methods: {
      partsAdministration(v){ //配件管理
        v.changeValue  = this.changeValue;
        console.log(v.changeValue)
        let obj={name: "渠道配件", path: "/product/channelParts"}
        this.$router.push({path: "/product/channelParts"});
        this.$store.dispatch("partsAdministrationObj",{partsData:v,title:"渠道配件"})
        console.log(this.$store.state.partsAdministrationObj,"asdfsadas")
      ...
      del(v,item,i){ //删除
        ...
        if(i==3){
          ...
          this.delData.title = "产品下架";
        }else if(i==1){
          this.delData.title = "产品上架";
        }
      },
      ...
      paramsData(){//列表參數
        return {params: {
          ...
          "name":this.serverTypeName,// 服务费类型

<template>
  ...
  <div v-show="isChannel" ref="AccountFrom" :model="account" :rules="rules" class="login-container">
    ...
    <!--<div class="loginBox">-->
      ...
      <!--&lt;!&ndash;<select placeholder="请选择城市" v-model="cityValue" @change="cityClick(cityValue)" style="width: 200px;height: 40px">&ndash;&gt;-->
        <!--&lt;!&ndash;<option v-for="(item,index) in options.options1"  :key="index"  v-bind:value="item.domain">{{item.cityName}}</option>&ndash;&gt;-->
      <!--&lt;!&ndash;</select>&ndash;&gt;-->
      <!--<el-select v-model="cityValue" placeholder="请选择城市" @change="cityClick(cityValue)" style="width: 268px;height: 40px" id="el-input__inner2">-->
        <!--&lt;!&ndash;<el-select v-model="cityValue" placeholder="请选择城市" style="width: 268px;height: 40px" id="el-input__inner2">&ndash;&gt;-->
            <!--<el-option-->
          ...
          let params={"username":this.account.username,"password":md5(this.account.pwd),"type":getSession("KEY")[0]};
           this.$http.post(url,params).then(res=>{
            ...
            if(data.code==="0000"){
              ...
              localStorage.setItem("access-user",JSON.stringify(data.result));  //另开页签用carter
              sessionStorage.setItem('access-user', JSON.stringify(data.result));
              setSession("userInfo",data.result);
              localStorage.setItem("userInfo",JSON.stringify(data.result));//另开页签用carter
              localStorage.setItem("enrolleeinfo",JSON.stringify(data.result));//另开页签用carter

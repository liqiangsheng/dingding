      ...
      <div>
        <div class="yuyue">
          ...
          <ul>
            ...
            <li>
              <div class="ziqudao">
                <p class="ziqudaoP">　主渠道<span>*</span>
                  ...
                  <span v-show="!value1||value1==''" >{{qudaoError}}</span>
                  　　子渠道:
            ...
            <table cellSpacing="0px" cellpadding="0">
              ...
              <tr v-for="(item,index) in tableListData" id="trHover">
                ...
                <td style="flex-grow:3">
                  {{item.serviceInfo.fullName}}<span v-if="item.serviceTags">:</span>{{item.serviceTags}}
                </td>
                <!--品牌-->
                <td>
                  {{item.serviceBrand}}
                ...
                <!--服务费-->
                <td>
                  {{item.serviceInfo.price2DiscountFee?item.serviceInfo.price2DiscountFee:item.serviceInfo.price2}}
                  <!--操作-->
                </td>
                <td>
        ...
        <section style="margin-bottom:2em;">
          ...
          <el-input v-model="payedPrice" type="number" min="1" @blur="payedPriceBlur(payedPrice)"></el-input>
        </section>
    ...
    methods: {
      payedPriceBlur(v){
        if(v<=1){
          ...
          this.payedPrice =Math.floor(v * 100) / 100
        }
      },
      goBack(){
        this.isShowCity = false;
      ...
      historyCarter(historyCarterLength) {  //新建订单页面历史记录显示数据
...
//            this.ziqudao = [{id: this.zhuqudaoId, name: "--请选择--"}, ...res.data.result];
            this.ziqudao = [{id:"", name: "--请选择--"}, ...res.data.result];
            this.disabled = false;
          }
        });
...
//          cityData.linkmanBuilding = this.addressData.value;
      ...
      getTableList(params) {
        ...
        this.$http.post(url, params).then(res => {
          if (res.data.code == "0000") {
            //充值显示
            this.$queryFun.successAlert.call(this, "恭喜订单新建成功", "1");
//            this.reload();
            ...
            let obj1={name: "新建订单", path: "/order/list0"}
            this.$store.commit("spliceTabByPath",obj1.path)
            let obj={name: "待分配订单", path: "/order/list2"}

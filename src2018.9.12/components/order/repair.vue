  ...
  <div id="app">
        ...
        <!--表格数据-->
          ...
          <tr v-for="(item,index) in tableList" :class="[{activeOrder:item.styleShow==true},{ trActive:item.borderShow == true}]" >
            ...
            <td class="icon" id="iconS">
              ...
              <!--------------------------------------------------------------------->
              <div v-if="item.state == '05'||item.state == '04'">
              ...
              <!------------------------------------>
              <ul class="iconShow" v-show="item.iconShow" @mouseleave="outHide(item,index)" v-if="item.state == '04'">
                <li>
    ...
    <!--处理订单-->
      ...
      <div class="alert" v-if="time1.isShow" >
        <yuyueTime :time1="time1" :quiry="quiry"  style="z-index: 100000;"></yuyueTime>
      </div>
<!--投诉订单 end-->
...
<script>
  ...
  //2017-11-29,李玉锋添加，全部订单的穿件回访弹出框组件，adComeBack.vue
  import AddComeBack from './orderAllCommonts/addComeBack.vue'
  import yuyueTime from "./orderAllCommonts/master_alert/yuyueTime.vue" //預約時間
  ...
  export default {
    ...
    data() {
      return {
        ...
        //订单详情Data
        orderDetailed:{
          isShow:this.$store.state.repairOrderId?this.$store.state.repairOrderId.isBool:true,
//          isShow:true,
          show:true,
          title:"基本信息"
        },
...
//            添加工程师详情结束
    ...
    watch:{
      "$store.state.repairOrderId":function(a,b){ //监听订单列表传过来的id
        console.log(a,"sadas")
        this.orderDetailed.isShow = a.isBool;
      },
      ...
      derivation(){ //导出
        ...
        let obj = {
          "isAppointment" :this.isAppointment,
            ...
            "mainOrderId":this.selectorBehindObj.mainOrderId, //返修订单号

<template>
    <div id="app">
        <h3>{{time.title}} <i @click="close"></i></h3>
      ...
      <div class="bottom">
        ...
        <el-button  @click="masterImposeOperate(time,changeTime.newTime)" type="primary">确定修改</el-button>
      </div>
    </div>
</template>
<script>
      ...
      props:["time","quiry"],
        ...
        methods: {
          ...
          masterImposeOperate(time,newTIme){
            if(newTIme == ""){
              return this.$queryFun.successAlert.call(this,"请填写改约时间")
            }
            ...
            this.$http.post(urlR,{"timestr":date,"orderid":time.orderId}).then(r=>{
              ...
              if(data.code==='0000'){
                time.isShow=false;
                this.$queryFun.successAlert.call(this,"改约时间成功")
                this.quiry();
              }else{

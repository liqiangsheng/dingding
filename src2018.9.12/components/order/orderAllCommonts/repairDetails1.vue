  ...
  <div id="app">
    <div v-if="orderDetailed.show">
      ...
      <ul class="alert_forbidden_text">
        <li>
          <span style="cursor: pointer"  @click="showorder(replacedata.data.mainOrderId)">订单号: {{replacedata.data.mainOrderId}}</span>
        </li>
        <li>
          返修订单号:  {{replacedata.data.id}}
        </li>
        <li>
        ...
        <li>
          ...
          <span class="time" v-show="show">
            <DatePicker v-model.trim="pricetime" type="datetime" format="yyyy-MM-dd HH:mm" placeholder="选择日期和时间"
                        style="width: 200px"></DatePicker>
          </span>
        </li>
        <li>
          服务工程师:  {{replacedata.data.masterName}}
        </li>
        <li>
          服务手机号:  {{replacedata.data.masterPhoneNum}}
        </li>
        <li>
          工程师工号:  {{replacedata.data.masterId}}
        </li>
        <li>

      ...
      <div class="essentialInfo">
        <div class="left">
          ...
          <table :class="{edit:this.isEdit()}">
            <tr>
              <td class="fontWidth">订单号</td><td  @click="showorder(orderData.mainOrderId)"><span style="cursor: pointer" class="orderLine">{{orderData.mainOrderId}}</span></td>
              <td class="fontWidth">状态</td><td><span style="color:rgba(230,88,49,1);">{{orderData.state | orderStateShow|placeholder}}</span></td>
            </tr>
            <tr>
              <td class="fontWidth">返修订单号</td><td> {{orderData.id}}</td>
            ...
            <tr>
              ...
              <td class="fontWidth">完结时间</td><td>{{orderData.serviceTimeComplete|moment('YYYY-MM-DD HH:mm:ss')}}</td>
            </tr>
            <tr>
              <td class="fontWidth">联系人</td><td> {{orderData.linkmanName||placeholder}}</td>
              <td class="fontWidth">订单耗时</td><td>{{orderData.completeTimeLimit}}</td>
            </tr>
            <tr>
      ...
      <div class="orderEvaluate">
        ...
        <table v-if="telescopic6">
          <tr style=" color:rgba(94,109,130,1);">
            <td>工程师工号</td><td>工程师名称</td><td>订单评分</td><td>评论标签</td><td>评论内容</td>
          </tr>
...
<script>
  ...
  export default {
    ...
    props:["orderDetailed","quiry",'drawbackIsShow','addRemarksIsShow','replaceMaster','complainFn',
     //订单报备     罚款工程师             改约时间         新增服务产品addSaverProduct        催单
    ...
    data(){
      return{
        ...
        dailyIsShow:{
          isShow:false,
          orderId:this.orderDetailed.orderId?this.orderDetailed.orderId:this.$store.state.repairOrderId.repairId
        },
        isCharge:{
          isShow:false
        },
    ...
    created(){
      ...
      let a =this.$store.state.repairOrderId.repairId?this.$store.state.repairOrderId.repairId:this.orderDetailed.orderId;
    ...
    methods: {
      ...
      showorder(id){
        let obj={name: "全部订单", path: "/order/list5"}
        this.$router.push({path: "/order/list5"});
        this.$store.dispatch("orderDetailiD",{mainOrderId:id,isBool:false,title:"全部订单"})
...
//        this.time.sum=sum;
      ...
      isalerShow(){
        ...
        this.$store.dispatch("orderDetailiD",{mainOrderId:"",isBool:true,title:"全部订单"})
      },
      charge(item){
...
//          alert("发请求")
    ...
    watch:{
      "$store.state.repairOrderId.repairId":function(newl,oldl){ //监听配件详情配件列表传过来的id
        console.log(newl,"dsajkdskadk")

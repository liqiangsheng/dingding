<template>
  <div id="app" v-cloak @touchmove.prevent>
    ...
    <div class="headerBtn bottom">
      ...
      <div>
        <p class="left_info_edit cursor"
           v-if="(dataObj.orderStatus=='05' || dataObj.orderStatus=='06' || dataObj.orderStatus=='08'|| dataObj.orderStatus=='21') && repair_work_btn"
           @click="have_arrived(dataObj.orderNumber)">
          工程师已抵达
        </p>
        <p class="cursor" v-if="(dataObj.orderStatus=='21' || dataObj.orderStatus=='04' || dataObj.orderStatus=='05' || dataObj.orderStatus=='06'||dataObj.orderStatus=='07'|| dataObj.orderStatus=='08'|| dataObj.orderStatus=='10'||
        ...
        <p class="cursor refresh" @click="refresh">刷新</p>
      </div>
      <div>
        <p class="cursor"
      ...
      >催单</p>
        <p class="cursor"
           v-if="orderData.state=='08'||orderData.state=='09'"
           @click="disposeOrder(orderData.id,orderData)"
        ><span v-show="orderData.state=='08'">处理</span><span v-show="orderData.state=='09'">确认完成</span></p>
    ...
    <div class="tableList">
      ...
      <div class="productInfo">
        ...
        <div v-if="telescopic">
          ...
          <table class="tableProduct" style="border-top: 1px solid #C0CCDA" v-for="(item,index) in productList.tableListData" :key="index"
                 :class="{active:item.status == 1,otherTable:index !==0,lastTable:index==(productList.tableListData.length-1)}">
            <!--<thead>-->
            <tr  style="color:rgba(94,109,130,1);background:rgba(240,240,240,1);">
              <td rowspan="2">产品{{index+1}}</td>
              <td v-for="(items,indexs) in theadList" :key="indexs">{{items}}</td>
            </tr>
            <!--</thead>-->
            <!--<tbody>-->
            <tr>
              <td>
                {{item.serviceFullName}} <span v-if="item.serviceTags">:</span> {{item.serviceTags}}
              </td>
              ...
              <td>
                ...
                <!--'型号',-->
                {{item.serviceModel ? item.serviceModel : ""}}
              </td>
              <!--折扣服务费-->
              <td>
                <del style="color:#888888;display: block" v-if="item.price2Original !==item.price2Sale">
                  ￥{{item.price2Original | placeholder_one}}
                </del>
                <span>￥{{item.price2Sale}}</span>
              </td>
              <!--券后服务费-->
              <td>
                <del style="color:#888888;display: block" v-if="item.price2Sale !==item.price2Voucher">
                  ￥{{item.price2Sale | placeholder_one}}
                </del>
                <span>￥{{item.price2Voucher}}</span>
              </td>
              <td>{{item.serviceSize | placeholder_one}}</td>
              <td>￥{{item.price2 * item.serviceSize.toFixed(2)}}</td>
              ...
              <td>
               <span
                 v-if="Date.parse(item.warrantyDatetime)>Date.parse(new Date())&&item.warrantyDatetime"
                 class="details_icon"><el-button type="text":disabled="item.status==1" @click="repair(item,index)"><i></i>返修</el-button></span>
                <span
                  v-if="$queryFun.isReturnCurrentButton(dataObj.orderStatus,item.isSecondPayment)&&item.status!=1"
                  class="details_icon"><el-button type="text":disabled="item.status==1" @click="returncash(item)"><i></i>返现</el-button></span>
                <!--暂时隐藏-->
                <span
                  v-if="$queryFun.isChangeSaverButton(dataObj.orderStatus)&&item.status!=1||orderData.state=='20'"
                  class="details_icon"><el-button type="text":disabled="item.status==1" @click="modify(item,index)"><i></i>改服务</el-button></span>
                <!--<span v-if="$queryFun.isPreferentialButton(dataObj.orderStatus,item.isSecondPayment)&&item.status!=1"-->
                <span v-if="orderData.state=='01'||orderData.state=='03'||orderData.state=='04'||orderData.state=='05'||orderData.state=='08'||orderData.state=='06'||orderData.state=='07'||orderData.state=='10'||orderData.state=='11'||orderData.state=='20'||orderData.state=='21'"
                  class="details_icon"><el-button type="text" :disabled="item.status==1" @click="charge(item,index)"><i></i>改价</el-button></span>
              </td>
            </tr>

            <tr style="height: 30px;background:rgba(240,240,240,1);color:rgba(94,109,130,1);" v-if="item.mountings&&item.mountings.length">
              <td>序号</td>
              <td colspan="3">配件类型</td>
              <td colspan="5">配件名称</td>
              <td colspan="2">数量</td>
              <td colspan="4">价格</td>
            </tr>
            <tr v-for="(todo,index) in item.mountings"
                :class="{onlyClass:true,firstClass:index===0,lastClass:index==(item.mountings.length-1)}" :key="index">
              <td>{{index+1}}</td>
              <td colspan="3">{{todo.masterId?"自定义配件":"平台配件"}}</td>
              <td colspan="5">{{todo.serviceMountingsName}}</td>
              <td colspan="2">{{todo.serviceMountingsCount}}</td>
              <td colspan="4">￥{{todo.serviceMountingsPrice}}</td>

            </tr>

            <!--</tbody>-->
          </table>
          <table class="tablePrice" style="border-top: 1px solid #C0CCDA">
            <tr>
              <td>上门费小计<span>￥{{orderData.price1Original|twoNum}}</span></td>
              <td>高空费小计<span>￥{{orderData.price4Original|twoNum}}</span></td>
              <td>服务费小计<span>￥{{orderData.price2Original |twoNum}}</span></td>
              <td colspan="2">配件费小计<span span style="left:87%">￥{{orderData.price3Original|twoNum}}</span></td>
              <td>订单合计<span>￥{{orderData.originalTotal|twoNum}}</span></td>
            </tr>
            <tr>
              <td>已付定金<span>￥{{orderData.payedPrice|twoNum}}</span></td>
              <td>折扣金额<span>￥{{orderData.price2Original-orderData.price2Sale|twoNum}}</span></td>
              <td>积分抵扣<span>￥0</span></td>
              <td>用券折扣<span style="left: 75%">￥{{orderData.discountPrice|twoNum}}</span></td>
              <td>改价金额<span>￥{{orderData.changePriceTotal|twoNum}}</span></td>
              <td>优惠合计<span>￥{{orderData.price2Original - orderData.price2Sale + orderData.discountPrice|twoNum}}</span></td>
            </tr>
            <tr>
              <td>已退款<span>￥{{orderData.refundAmount|twoNum}}</span></td>
              <td>已返现<span>￥{{orderData.cashbackAmount|twoNum}}</span></td>
              <td>实际已收款<span>￥{{orderData.paidAmount|twoNum}}</span></td>
              <td>渠道待付款<span>￥{{orderData.pendingFee|twoNum}}</span></td>
              <td>用户待付款<span>￥{{orderData.waitAmount|twoNum}}</span></td>
              <td>应付合计<span>￥{{orderData.total|twoNum}}</span></td>
            </tr>
          </table>
        </div>
      </div>
      ...
      <div class="essentialInfo">
          ...
          <div v-if="telescopic1">
            ...
            <table :class="{edit:this.isEdit()}">
              <tr>
                ...
                <td>订单号</td>
                <td>{{dataObj.orderNumber || placeholder}}</td>
                <td>订单类型</td>
                <td>{{dataObj.type | orderType}}</td>
                <td>状态</td>
                <td>{{dataObj.orderStatus | orderStateShow | placeholder}}</td>
              </tr>
              <tr>
                <td>订单来源</td>
                <td>{{dataObj.orderSource | orderSourceShow | placeholder}}</td>
                <td>紧急程度</td>
                <td>{{orderData.emergencyDegree | jinjidu}}</td>
                <td>已收定金</td>
                <td :class="{Deposit:editPayedPrice&&isEdit()}">
                  <span v-if="!isEdit()  || !editPayedPrice ">{{orderData.payedPrice || 0}}</span>
                  <el-input v-if="editPayedPrice&&isEdit()" v-model.trim="orderData.payedPrice" min="0"
                            ...
                            @blur="payedPriceBlur(orderData.payedPrice)"
                            type="number"></el-input>
                </td>
              </tr>
              <tr>
                <td>订单渠道</td>
                <td>{{dataObj.officialPartnerSubsetName | placeholder}} </td>
                <td>渠道商订单号</td>
                <td>{{dataObj.thirdOrderId | placeholder}}</td>
                <td>渠道质保</td>
                <td :class="{Deposit:isChannel&&isEdit()&&orderData.channelSettleType=='1'}">
                  <!--渠道质保:{{orderData.channelWarranty|qudaozhibao}}-->
                  <span v-if="!this.isEdit() || !isChannel||orderData.channelSettleType!='1'">
              ...
              <tr>
                ...
                {{dataObj.contactsName || placeholder}}
                  ...
                  <input
                    ...
                    style="width: 100%;height:30px;border:none;margin-left:-5px;outline:none;">
              </span>
                </td>
                <td>联系人手机号</td>
                <td>
                ...
                <td>绑定手机号</td>
                <td>{{dataObj.userNumber || placeholder}}</td>
              </tr>
              <tr>
                <td>服务城市</td>
              ...
              <tr>
                <td>预约时间</td>
                <td>{{dataObj.makeTime || placeholder}}</td>
                <td>下单时间</td>
                <td>{{orderData.createTime | moment('YYYY-MM-DD HH:mm:ss')}}</td>
                <td>完结时间</td>
                <td>{{orderData.serviceTimeComplete | moment('YYYY-MM-DD HH:mm:ss')}}</td>
              </tr>
              <tr>
                <td>网点名称</td>
                <td colspan="5">{{orderData.site || "无"}}</td>
              </tr>
              <tr>
                <td>工程师姓名</td>
                <td>{{dataObj.masterName | placeholder}}</td>
                <td>工程师类型</td>
                <td><span v-if="!!masterInfos"> {{masterInfos.type | masterType}}</span> <span v-else>无</span></td>
                <td>工程师工号</td>
                <td>{{dataObj.masterId || placeholder}}</td>
              </tr>
              <tr>
                <td>工程师手机号</td>
                <td>{{dataObj.masterPhoneNum | placeholder}}</td>
                <td>订单耗时</td>
                <td>{{orderData.completeTimeLimit}}</td>
                <td>准时偏差值</td>
                <td>{{orderData.deviation}}</td>
              </tr>
              <tr>
                <td>异常次数</td>
                <td>{{orderData.abnormalHangSize}}</td>
                <td>挂起次数</td>
                <td>{{orderData.hangSize}}</td>
                <td>催单次数</td>
                <td>{{orderData.reminderSize}}</td>
              </tr>
              <tr>
                <td>返修次数</td>
                <td>{{orderData.reworkSize}}</td>
                <td>预约次数</td>
                <td>{{orderData.appointmentSize}}</td>
                <td>服务时长</td>
                <td>{{orderData.workTimeLimit}}</td>
              </tr>
        ...
        <div class="right  ">
          <p>
            <span @click="takeUp(data='订单额外信息')" class="cursor">
            ...
            <table>
              ...
              <tr v-for="(item,index) in tableListData.orderExceptionRecords" :key="index">
                <td colspan="3">
                  ...
                  <p>
                    <span style="color:blue;cursor:pointer;" v-if="!!item.idPhotos"
                          @click="openImage(item)"> 查看图片</span>
      ...
      <div class="tradeInfo">
        ...
        <table v-if="telescopic3">
          ...
          <tr v-for="(item,index) in payRecordList.tableListData" v-if="payRecordList.tableListData.length">
            ...
            <td>
              <span v-if="item.payType=='Pay'">{{item.feeType | payTypeShow}}</span>
              <span v-if="item.payType=='Refund'">{{item.refundType | payTypeShow}}</span>
            </td>
            <td>
      ...
      <div class="orderProof">
        <p>
            <span @click="takeUp(data='订单凭证信息')" class="cursor">
        ...
        <div v-if="telescopic4">
          <div class="photoes" v-for=" (item,index) in isPotos.potosimg" v-if="isPotos.potosimg.length">
            ...
            <div class="bottom">
              <p>拍照时间:<span
                v-if="isPotos.idPhonesTime[index]">{{isPotos.idPhonesTime[index] == "null" ? "无" : isPotos.idPhonesTime[index]}}</span>
                <span v-else>无</span>
              </p>
              <p>
      ...
      <div class="orderEvaluate">
        <p>
          <span @click="takeUp(data='订单评价')" class="cursor">
        ...
        <table v-if="telescopic6">
          ...
          <tr v-for="(item,index) in orderAppraise.tableListData" :key="index"
              v-if="orderAppraise.tableListData.length">
            <td>{{item.masterId}}</td>
    ...
    <div class="alert" v-if="mapData.isShow">
      <position-map style="z-index: 999999;width:50%;height:70%;margin:0 auto;transform:translateY(20%);"
                    :mapData="mapData"></position-map>
    </div>
...
<script>
  ...
  export default {
    ...
    props: ["orderDetailed", "quiry", 'drawbackIsShow', 'addRemarksIsShow', 'replaceMaster', 'complainFn',
      //订单报备     罚款工程师             改约时间         新增服务产品addSaverProduct        催单 处理
      'unusual', 'masterImposeIsShow', 'changeTimeIsShows', 'reminder', 'yuYueTimeIsShow','disposeOrder'
    ],
    components: {
      alertInfo,
      changTime,
    ...
    data() {
      return {
        ...
        btnList: [
          {name: '工程师已抵达', id: '0'}, {name: '订单已完成', id: '1'}, {name: '工程师位置', id: '2'}, {name: '订单日志', id: '3'}
        ],
        operationList: [
          {name: '订单报备', id: '0'}, {name: '改派', id: '1'}, {name: '新增产品', id: '2'},
          {name: '备注', id: '3'}, {name: '投诉订单', id: '4'}, {name: '取消订单', id: '5'},
          {name: '催单', id: '6'}, {name: '罚款工程师', id: '7'}, {name: '订单报备', id: '8'},
        ],

        theadList: [ '产品名称', '服务名称', '品牌型号', '折后服务费(元)', '券后服务费(元)',
          '产品数量', '应付总服务费', '实收总服务费', '上门费(元)', '高空费(元)', '实收合计', '质保截止', '操作'],
        tradeTheadList: ['交易流水号', '交易时间', '交易类型', '交易方式', '交易金额', '交易状态'],
...
//          }
        ...
        repair_work_request_again: false,  //在订单已完成做回调请求
    ...
    computed: {
      ...
      orderDetailsState() {  //工程师已抵达和订单已完成的按钮是06和07就显示，否则隐藏
    ...
    methods: {
      ...
      payedPriceBlur(v){
        if(v<=0){
          this.orderData.payedPrice = 0;
        }else{
          this.orderData.payedPrice = Math.floor(v * 100) / 100
        }
      },
      parseDetail(v) {
      ...
      findOne() {
        ...
        this.$http.get(url, params1).then(r => {
          ...
          if (data.code === '0000') {
            ...
            if (res.state === '03' || res.state === '04' || res.state === '05' || res.state === '06' || res.state === '07' || res.state === '08' || res.state === '21' || res.state === '20'|| res.state === '10') {
              this.isChannel = true;
            }
            this.orderData = data.result.coreMainOrder;
...
//        this.time.sum=sum;
      ...
      isalerShow() {
        ...
        this.$store.dispatch("orderDetailiD", {data: "", isBool: true, title: "全部订单"})
      },
      charge(item) {
      ...
      enter() {
        ...
        this.$http.post(url, Object.assign(o, this.addressCode(this.addressData.addressComponent))).then(res => {
          ...
          if (data.code === '0000') {
            ...
            /*if (isChangeAreaId) {
              this.$queryFun.successAlert.call(this,"地址已发生修改，请注意是否需要更换工程师!!!");
            }*/
            this.findOne();
            this.quiry();
            this.cancelEdit();
      ...
      have_arrived(id) { //工程师已抵达
        ...
        this.repair_work_text = '请确定修改订单到工程师已抵达'; //显示内容
        this.repair_work_show = true;
      },
      completed(id) { //订单已完成
        this.repair_work_id = id; //当前的ID
        this.repair_work_text = '请确定修改订单到已完成'; //显示内容
        this.repair_work_show = true;
      },
      ...
      repair_work_determine_click() {      //弹出框确定
        this.repair_work_show = false;
        if (this.repair_work_text == '请确定修改订单到工程师已抵达') {
          this.have_arrived_request();//工程师已抵达的请求
        }
        if (this.repair_work_text == '请确定修改订单到已完成') {
      ...
      completed_request() { //工程师已完成的请求
        ...
        _this.$http.post(url, parmas).then(data => {
          if (data.data.code == '0000') {
            ...
            _this.repair_work_btn = false;  //隐藏按钮
          } else if (data.data.code == '3010') { //如果返回结果没有成功就在调用此接口
            ...
            this.$queryFun.successAlert.call(this, data.data.error);
          }
        });
      },

<template>
  <div id="app">
          ...
          <tr v-for="(item,index) in tableListData.list" :class="[{activeOrder:item.styleShow==true},{ trActive:item.borderShow == true}]" >
            ...
            <td class="icon" id="iconS">
              ...
              <div v-if="item.state == '02'">
                <span class="iconStyle"  @click="addRemarksIsShow(item)">备注</span>
                <span class="iconStyle" @click="orderDetails(item,item.id,index)">订单详情</span>
              </div>
              <!--------------------------------------------------------------------->
              ...
              <div v-if="item.state == '09'">
                <span class="iconStyle"  @click="addRemarksIsShow(item)">备注</span>
                <span class="iconStyle"  @click="disposeOrder(item.id,item)">确认完成</span>
                <span class="iconStyle"   @click="more(item,index)" style="margin:0 6px;border-radius:2px;display: inline-block;color: #fff;background: #23A1FE;padding: 3px 8px">更多</span>
                <!--详情-->
              </div>
              ...
              <div v-if="item.state == '10'||item.state == '11'">
                <span  class="iconStyle"  @click="drawbackIsShow(item)">取消</span>
                <span class="iconStyle"  @click="addRemarksIsShow(item)">备注</span>
                <span  class="iconStyle" @click="unusual(item.id)">订单报备</span>
                <span class="iconStyle"   @click="more(item,index)" style="margin:0 6px;border-radius:2px;display: inline-block;color: #fff;background: #23A1FE;padding: 3px 8px">更多</span>
                <!--详情-->
              </div>
    ...
    <!--退款-->
      <drawback :isDrawback="isDrawback" :data="drawbackData" v-if="isDrawback.isShow" :quiry="quiry" @isClosed="isClosed">
        ...
        optionList: [       //下拉列表数据
          {
            ...
            SourceType:[{value:"网点待分配",id:"04"},{value:"待上工",id:"05"},{value:"待抵达",id:"06"},{value:"待分配",id:"03"},{value:"服务中",id:"07"},{value:"待核账",id:"10"},{value:"待付尾款",id:"11"},{value:"待退款",id:"13"},{value:"待预约",id:"21"}]
          }
        ],
        stateOne: "04,05,06,07,10,11,13,21",
        selectorBehindObj: {},
        theadsName: [
          '序号',
          '订单号',
    ...
    methods: {
      ...
      exportOrder(){ //导出
        ...
        if (this.timeQuantum3.length > 0) {
          ...
          this.completedTimeEndStr = this.$moment(this.timeQuantum3[1]).format('YYYY-MM-DD HH:mm:ss').includes("Invalid")?"":this.$moment(this.timeQuantum3[1]).format('YYYY-MM-DD');
        }
  ...
  //新增服务产品 end
      ...
      paramsData(){
        ...
        return{params:{
          ...
          "source":this.numId1, //订单来源
          "siteId":this.siteID, //网点名称
          "orderByClause": "send_order_time desc",
        }};
      },
      //显示新建订单；
      addOrderIs(){

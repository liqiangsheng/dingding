      ...
      <ul class="centent1">
        ...
        <li>
          ...
          <table v-show="jiantouShow2" id="parseInfo1">
            <thead>
            <tr>
              <td>订单号</td><td>联系人</td><td>联系人手机号</td><td>服务城市</td><td>服务地址</td><td>订单渠道</td><td style="width: 60px">渠道<br>质保</td><td>申领时间</td><td>工程师姓名</td><td>工程师手机号</td>
            </tr>
            </thead>
            <tbody>
            <tr>
        ...
        <li>
          ...
          <table  cellSpacing="0" cellpadding="0" v-show="jiantouShow3" id="parseInfo2">
            ...
            <tr v-for="(item,index) in mountingsApplyLogList">
              ...
              <td>
                {{item.operatorRole}}-{{item.operator}}
              </td>
              <td>
  ...
  export default {
    ...
    data() {
      return {
        ...
        ParseID:this.$store.state.parseDetailiD.id,//要么列表的ID要么订单详情的ID
        isPotos:{//照片
          isShow:false,
          potosimg:[],
          idPhonesTime:[],
    ...
    methods: {
      orderClick(v){ //
        ...
        this.$store.dispatch("orderDetailiD",{mainOrderId:v.mainOrderId,isBool:false,title:"全部订单"})

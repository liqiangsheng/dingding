...
<template>
       ...
       <ul>
          <li class="add_master">
            <li>
              ...
              <el-input
                placeholder="请填写内容"
                v-model.trim="selectorBehindObj.saleName"
              >
              </el-input>
            ...
            <li>
              ...
              <DatePicker v-if="isEditInput" @on-change="initDate" v-model.trim="selectorBehindObj.date"
                          type="datetimerange" format="yyyy-MM-dd HH:mm:ss" placeholder="请选择时间"
            ...
            <li>
              ...
              <el-radio-group v-model.trim="radio2" @change="radio">
                <el-radio :label="1">平台承担</el-radio>
                <el-radio :label="2">平台网点和工程师共担</el-radio>
              </el-radio-group>
            </li>
 ...
 <!--渠道  start-->
         ...
         <el-checkbox v-if="isEditInput" @change="checkAll(checkeAll)" style="transform:translateX(20px);"
                      v-model.trim="checkeAll">全部通用
         </el-checkbox>
  ...
  <!--渠道  end-->
          ...
          <div  v-for="(item,index) in searchOptionList" :key="index" class="list">
            ...
            <el-select
              ...
              @change="selectorSearch(item,item.SourceType,item.SourceTypeValue)">
              <el-option
          ...
          <div  class="list">
            ...
            <el-input v-model.trim="searchParams.fullName"></el-input>
          </div>
...
<!--列表 start-->
          <div class="harder">
            ...
            <div class="right">
              ...
              <el-select
                style="width:150px;"
                v-model.trim="rebateType.SourceTypeValue" placeholder="启用百分比设置"
                @change="selector(rebateType.SourceTypeValue)">
                <el-option
              ...
              批量设置折扣 :
              <el-input style="width:90px;" type="number" :max="isRebateType?520:10"
                        v-model.trim="rebateSum"></el-input>
              {{isRebateType ? typeTexts[1] : typeTexts[0]}}
          ...
          <table  id="table" cellSpacing="0px" cellpadding="0">
            ...
            <tr class="theads">
              <th>
                <el-checkbox :disabled="!tableList.length" v-model.trim="checked"
                             @change="wholeSelector(tableList,checked)"></el-checkbox>
              </th>
            ...
            <tr v-for="(item,index) in tableList">
              <td>
                <el-checkbox :disabled="!isAddProduct(item.saleState,index)" v-model.trim="isCheckboxList[index]"
                             @change="isCheckbox(item,index)"></el-checkbox>
              </td>
              <td>
                {{item.cityName}}
              </td>
              ...
              <td class="icon">
                   <span>
                     <el-button @click="lookOver(item)"  type="text">
                      查看折扣
                      </el-button>
                   </span>
...
//批量添加  end ;
      ...
      lookOver(e){
        ...
        this.coolkRebateAlert.id=e.serviceId;

        this.coolkRebateAlert.areaId=e.cityCode
...
//            this.loading2=true;
            let url=`${this.$apidomain}/sale/findServiceInSale`, params={
              ...
              "channelId":this.selectorBehindObj["channelId"]||"ALL",     //渠道 Id;
...
//        设置折扣
      ...
      getArticleinfo(){
        let url=this.$apidomain+"/common/findChannelLists";
        this.options=[];
      ...
      addData(){
       ...
       //结算方式；
        ...
        if(exp.test(this.selectorBehindObj["saleName"])){
          return this.$queryFun.successAlert.call(this," 请填写正确的折扣名称，不得出现特殊字符");
        }
        if(!this.selectorBehindObj["saleName"]){                                                        //折扣名称
          return this.$queryFun.successAlert.call(this,"请填写正确的折扣名称，不得超过26个字符","0")
...
//        }else
          if(this.selectorBehindObj["limitCollar"]<0){
        ...
        let url=`${this.$apidomain}${domain}`,params={"id":this.subMitId,"saleId":this.saleId};
            ...
            console.log(this.$store.state.rightTableList,"adasdasdas")
    ...
    watch:{
        ...
        "selectorBehindObj.saleName":function (current){
          let exp=/[`‘；“：,，~!@#$%^&*_+<>{}\/'[\]]/g;
          if(exp.test(current)){
            this.NameText="请填写正确的折扣名称，不得出现特殊字符"
          }else{
            this.NameText=null;
          }
        }

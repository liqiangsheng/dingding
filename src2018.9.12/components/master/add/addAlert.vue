    ...
    <div class="container">
      ...
      <div class="scrollbar content">
        <div class="left_info">
          ...
          <ul class="add_master">
            <li v-if="!!isAdd.id">
              <span class="list_name">工程师编号</span>
              <span class="editStyle">{{editData.masterInfo.id}}</span>
            </li>
            <li>
              ...
              <span v-show="nameOne&&!isAdd.id"  class="reg">{{nameOne}}</span>
            </li>
            <li>
            ...
            <li>
              <span class="list_name"> 工程师类型
                <b style="color: #EA5413" v-if="isEdit">*</b>
              </span>
              <el-select
                v-if="isEdit"
                v-model.trim="type.SourceTypeValue" placeholder="请选择工程师类型"
            ...
            <li style="width: 100%">
              <p class="input_title" style="margin: 0;margin-bottom: 24px;">认证资料 </p>
            ...
            <li v-if="isAdd.id==''" style="margin-right: 0;margin-top: 24px">
              <ul class="files">
                <li v-for="(item,index) in files" :key="index">
                  <a href="javascript:;" class="file">
                    <input type="file" @change="onchangeFile($event,item.key,item)" accept="image/gif,image/jpeg,image/jpg,image/png,image/svg">
                    <span v-if="item.flag"></span><span v-if="item.flag"></span>
                  </a>
                  {{item.name}}
                </li>
              </ul>
      ...
      if(editData.id != ""){
        ...
        this.$http.get(url).then(r=>{
          ...
          if(data.code==="0000"){
            ...
            console.log(this.editData,'工程师向国庆')
            //开户行
            ...
            if(this.editData.masterInfo.level === "Primary"){
              this.level.SourceTypeValue = "初级工程师"
            }else if(this.editData.masterInfo.level === "MiddleRank"){
              this.editData.masterInfo.SourceTypeValue="中级工程师"
            }else if(this.editData.masterInfo.level === "Advanced"){
              this.level.SourceTypeValue= "高级工程师"
            }
            //工程师类型
            let masterType='';
            switch (this.editData.masterInfo.type){
              case "1":masterType="自营工程师";this.type.SourceTypeValue="自营工程师";break;
              case "2":masterType="兼职工程师";this.type.SourceTypeValue="兼职工程师";break;
              case "3":masterType="网点工程师";this.type.SourceTypeValue="网点工程师";this.getSiteList();break;
            }
            //城市
            ...
            let imgOne = images.slice(3,images.length);
...
//            });
        ...
        handler:function(new1,old1){
          if(new1[0].value != ""){
            ...
            if(res2.test(new1[0].value)==false){
              this.nameOne = "请填写工程师姓名，长度不超过20字符，不可输入特殊符号";
            }else{
              this.nameOne = "";
            }
          ...
          if(new1[3].value != ""){
            ...
            if(res2.test(new1[3].value)==false){
              this.frindrenOne= "请填写朋友姓名，长度不超过20字符，不可输入特殊符号";
            }else{
              this.frindrenOne = "";
            }
          ...
          if(new1[1].value != ""){
            ...
            if(reg.test(new1[1].value)==false){
              this.phoneNumOne = "请填写正确工程师手机号，长度为11位数字";
              return
            }else{
              this.phoneNumOne = ""
            }
    ...
    methods: {
      ...
      accomplish(prams,prams1){
//        console.log(this.editData.masterInfo.idPhotos,"asda")
        console.log(prams,prams1,"asda")
//        prams.idPhotos = this.editData.masterInfo.idPhotos.join(",");
        prams.idPhotos = prams1;
        prams.id=this.isAdd.id;
        prams.city=this.editData.masterInfo.city
...
//        if(this.isAdd.id != "")return this.accomplish(this.selectorBehindObj);
        ...
        if(isEdit)return this.accomplish(this.selectorBehindObj,this.editData.masterInfo.idPhotos);
        this.selectorBehindObj.idPhotos = this.selectorBehindObj.idPhotos.join(",");

        if(!this.selectorBehindObj.name){
          this.nameOne = "请填写工程师姓名";
          return
        }else{
          var res1 = /^[\u4e00-\u9fa5a-zA-Z0-9]{1,20}$/;
        ...
        if(!this.selectorBehindObj.level){
          this.levelOne = "请填写工程师等级";
          return
        }else{
          this.levelOne = ""
        }
        if(!this.selectorBehindObj.type){
          this.typeOne = "请填写工程师类型";
          return
        }else{
          this.typeOne = ""
        }
...
//        }
      ...
      onchangeFile(event,keyImg,item){
...
<style scoped lang="scss">
  ...
  .file {
    ...
    span{
      ...
      width: 45px;
      height: 45px;
      border-right: 4px rgba(216,216,216,1) solid;
      border-radius:2px;
      display: inline-block;
    }
    span:nth-of-type(1){
    ...
    span:nth-of-type(2){
      border-right: none;
      border-bottom: 4px rgba(216,216,216,1) solid;
      top:20px;
      left: 82px;
    }
  }
  .file:hover{
    border: 1px solid #23A1FE ;
    background: #F0F0F0;
  }
  .file input {
    background: #F0F0F0;

  ...
  export default {
    ...
    methods: {
      ...
      changeSelector2(val){
        ...
        let urlThree=this.$common.apidomain+"/articleinfo/findChannelListServiceInfo?labelId="+this.moziID +"&areaId="+this.cityId+"&relevanceKey="+this.xiaoqudaoId;
...
//      },
    ...
    created(){
      //分类同步请求分类
      const getFindLabel = (urlTwo,index) => this.$http.get(urlTwo).then(res1 => {
        ...
        if(res.data.code == "0000"){
          async function getData(){
            ...
            for(let i=0;i<this.fenleiOptions.length;i++){
              let urlTwo = this.$common.apidomain + '/articleinfo/findlabelbusinessbyflabel?id=' + this.fenleiOptions[i].id;
              await getFindLabel(urlTwo,i)
            }
          }
          getData.call(this);
        }else{

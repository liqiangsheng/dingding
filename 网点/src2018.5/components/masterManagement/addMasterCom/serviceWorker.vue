<template>
  <div class="addMasterList">
    <C_header title="服务工种" right_number="1" :addReset='resetWorker' left_number='3'></C_header>
...
//        workerLists: [],      //上一次选中的小类状态
    ...
    created() {
      this.getWorkList();
      // console.log(this.$store.getters.skills, '//工种')
    },
    watch: {},
    methods: {
      //服务工种数据
      getWorkList() {
        //console.log(this.$route.query.workerData, '111')
...
//        this.$router.push({path:`/addMaster/${JSON.stringify(this.workerList)}`})
        storage.setSession("workerList", this.workerList)
        this.$router.push({path: `/addMaster`});

        // this.$router.push({path: `/addMaster`, query: {workerData: this.workerList, areaData: this.areaList}})

//        this.$router.push({path:`/editMasterDetails`,query:{workerData:this.workerList}})
      }
    }
  }
</script>

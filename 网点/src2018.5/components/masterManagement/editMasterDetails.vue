<template>
  <div class="addMasterList">
    ...
    <div class="bottom">
      <p>注册时间: {{registerTime | moment('YYYY-MM-DD HH:mm:ss')}}</p>
      <p>认证时间: {{certificateTime | moment('YYYY-MM-DD HH:mm:ss')}}</p>
    </div>
    <div v-if='isShow'>
...
//        this.setStore();
//        this.$router.push({path: '/serviceAreaEdit', query: {workerData: this.workerList, areaData: this.areaList}});
...
//        this.$router.push({path: `/serviceWorkerEdit`, query: {workerData: this.workerList, areaData: this.areaList}});
      ...
      changeData(arr1, arr2) {
        if (arr1 === [] || !arr1) {
          ...
          this.$http.get(url).then(res => {
            ...
            //console.log(arr1, '4444444444444444')
          })
        }
      },
      //时间戳转换
      ...
      getMasterDetails() {
        ...
        this.$http.get(url).then(res => {
          let data = res.data;
          if (data.code === "0000") {
            // console.log(data.result,'777777777')
            ...
            if (this.areaList === [] || !this.areaList) {
              // console.log('进来了')
              let url = _this.$common.apidomain + "/common/findareaandstreetoptions" + '?cityId=0101';  //服务工种列表
              _this.$http.get(url).then(res => {
                let data = res.data;
                if (data.code === "0000") {
                  this.areaList = data.result;
                  // console.log(this.areaList, '22222')
...
//                      obj.label = obj.label.substring(5);
                ...
                console.log(this.areaList, '8888888')
              })
            }
            ;
...
//                  console.log(this.workerList, '22222')
                ...
                console.log(this.workerList, '4444444444444444')
              })
            }
            ;

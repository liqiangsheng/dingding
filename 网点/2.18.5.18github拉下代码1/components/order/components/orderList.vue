  ...
  <div id="box" :class="{data_margin_top:id==='5'}">
    ...
    <section class="data_item"  v-for="(item,index) in orders" :key="index"  >
      ...
      <section class="operate_content" :class="{'operate_content_id':operateContentId(item.state,index)}">
        ...
        <router-link tag="div" :to="appointmentDatetime(item)" class="operate_content_item order_expo" v-if="item.state==='04'&&id!=='2'"> 改派 </router-link>
      </section>
    </section>
...
// 列表参数
      params(){
        ...
        switch(this.id)
        {
          ...
          case "2": state="04".split(",");break;
          case "3": state="05,06,07,10,11,13".split(",");break;
          case "4": state="08,09".split(",");break;
          case "5": state="09,12,14,15,17,18".split(",");break;
        }
        /*
        * 1 监控
        * 2 待分配

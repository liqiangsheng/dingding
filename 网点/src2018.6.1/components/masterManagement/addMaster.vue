    ...
    <div class="inputList">
      <ul>
        <li v-for="(item,index) in inputList" :key="index">
          ...
          <label class="isPhone" v-if="index===1&&!item.value||index===4&&!item.value">请填写手机号 <span><img
            src="../../assets/images/phone.png"></span></label>
          <label class="isPhoneNum"
                 v-if="index===1&&item.value&&!isPhone||index===4&&item.value&&!isPhoneNum">手机号有误 <span><img
            src="../../assets/images/phone.png"></span></label>
...
//      console.log(this.$route.query.areaData,'新建服务区域query')
//      console.log(this.$route.query.areaData,'转换query')
      //console.log(this.areaList, '新建服务区域session')
    ...
    watch: {
      inputList: {
        handler: function (newVal, oldVal) {
          // console.log(newVal[1].value,oldVal[1].value)
...
//        }
        ...
        this.setStore();
        this.$router.push({path: `/serviceArea`, query: {areaData: this.areaList, workerData: this.workerList}});
...
//        this.$router.push({path:`/addMaster`,query:{data:this.workerList}})
      ...
      //提交
      submit() {
//          console.log(this.inputList[0].value,'11111111')
//          this.inputList.forEach((v,i)=>{
//            if(!v.value){
//              this.$Toast(v.label+"不能为空");
//              return;
//            }
//          })
        ...
        if (!params.name || params.name === "") {
          this.$Toast(this.inputList[0].label + '不能为空');
          return;
        } else if (!params.phoneNum || params.phoneNum === "") {
          this.$Toast(this.inputList[1].label + '不能为空');
          return;
        } else if (params.phoneNum && !this.isPhone) {
          this.$Toast(this.inputList[1].label + '格式不对');
          return;
        } else if (!params.idNum || params.idNum === "") {
          this.$Toast(this.inputList[2].label + '不能为空');
          return;
        } else if (!params.emergencyContact || params.emergencyContact === "") {
          this.$Toast(this.inputList[3].label + '不能为空');
          return;
        } else if (!params.emergencyPhoneNum || params.emergencyPhoneNum === "") {
          this.$Toast(this.inputList[4].label + '不能为空');
          return;
        } else if (params.emergencyPhoneNum && !this.isPhoneNum) {
          this.$Toast(this.inputList[4].label + '格式不对');
          return;
        } else if (!params.areaIds || params.areaIds === "") {
          this.$Toast('服务区域不能为空');
          return;

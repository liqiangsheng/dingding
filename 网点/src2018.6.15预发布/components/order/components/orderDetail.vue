<template>
  ...
  <div style="position: absolute;left: 0;top:-0.96rem;bottom: 0;right: 0;">
    <C_header title="工单详情" left_number='2' :goRemark='goRemark' right_number='5'></C_header>
    <OrderTracking v-if="OrderTrackingShow" @isClose="isClose" :trackAlterId="trackAlterId"></OrderTracking>
    <priceDetail @contentFooterOne="contentFooterOne" v-if="contentFooter1"></priceDetail>

    <div id="orderDetail">
      <div class="orderDetail_title" @click="OrderTracking(orderInfo.id)">
        <p><span>{{orderInfo.state|BackOrderStatus}}</span>
          <img src="../../../assets/images/gobackTwo.png">
        </p>
      </div>
          <div class="orderDetail_header">
            <ul class="orderDetail_headercenter">
              <li style="padding-bottom:0.16rem "> <p><span>工单号：{{orderInfo.id}}</span><img :src="orderInfo.channelWarranty|channelWarranty" alt=""></p></li>
              ...
              <li id="price">
                     <p><span>服务费</span><span>￥{{orderInfo.price2}}</span></p>
                      <p><span>检测费</span><span>￥{{orderInfo.price1}}</span></p>
                      <p><span>高空费</span><span>￥{{orderInfo.price4||0}}</span></p>
              </li>
            </ul>
        ...
        <div class="orderDetail_content">
          ...
          <ul class="orderDetail_contentContent">
               <p id="Bailout"><img style="display: inline-block;width: 0.72rem;height: 0.33rem" v-if="orderInfo.emergencyDegree==1" src="/static/images/urgent.png"></p>
                <li><span>预约时间</span><span>{{orderInfo.appointmentDatetime|moment('YYYY-MM-DD HH:mm:ss')}}</span></li>
                <li><span>完成时间</span><span>{{orderInfo.serviceTimeComplete|moment('YYYY-MM-DD HH:mm:ss')}}</span></li>
                <li><span>准时偏差</span><span>{{orderInfo.deviation||"0"}}分钟</span></li>
                <li><span>备注</span><span>{{orderInfo.remark||"无"}}</span></li>
          </ul>
        </div>
      <ul  class="orderDetail_contentMaster" v-if="masterInfoShow">
        <li>师傅信息</li>
        <li><span>{{masterInfo.name}}</span><span>{{masterInfo.phoneNum}}</span></li>
        <li><span>师傅编号</span><span>{{masterInfo.id}}</span></li>
      </ul>
      ...
      <ul  class="orderDetail_contentFooter" v-show="contentFooter">
...
<script>
  ...
  import OrderTracking from "./orderTracking.vue"
    export default {
    components:{
      priceDetail,OrderTracking
    },
        data() {
            return {
              trackAlterId:"", //跟踪id
              OrderTrackingShow:false, //订单跟踪
              ...
              mountings:[],
              masterInfo:{},
              masterInfoShow:false,
            }
        },
        methods: {
          isClose(v){
            this.OrderTrackingShow = v;
          },
          OrderTracking(v){//订单跟踪
             this.OrderTrackingShow = true;
            this.trackAlterId = v;
          },
          goRemark() {
            this.$router.push({path: '/masterPosition/' + this.$route.params.id})
          },
          priceDetail(){ //点击显示详情
...
//          http://admin.test.dingdingkuaixiu.com/orderquery/findOneDetails?id=201805140938422068247698
          let url=`${this.$common.apidomain}/orderquery/findOneDetails?id=${this.$route.params.id}`
          this.$http.get(url).then(res=>this.$httpFilter(res).then(data=>{

            this.orderInfo = data.result.coreMainOrder;
           console.log(this.orderInfo)
            if(!data.result.masterInfo){
               this.masterInfoShow = false;
            }else {
              this.masterInfoShow = true;
            }

<template>
  <div id="app">
    <div class="container" :class="{master:selectorBehindObj.type==='2'}" >
      ...
      <div class="content">
        <div class="left_info">
          <ul>
            <li>
            <div class="list_name">
              推送对象<b style="color: #EA5413">*</b>
            </div>
            <el-select
              style="width:220px;"
              v-model.trim="pushObject.SourceTypeValue" placeholder="请选择推送对象"
              @change="selector(pushObject,pushObject.SourceType,pushObject.SourceTypeValue)">
            <el-option
            v-for="items in pushObject.SourceType"
            :key="items.value"
            :value="items.value"
            >
            </el-option>
            ...
            <li >
              <span class="list_name"> 广告名称<b style="color: #EA5413">*</b></span>
            ...
            <li>
              <span class="list_name"> 广告栏位置 <b style="color: #EA5413">*</b></span>
              <el-select
                style="width:220px;"
                v-model.trim="showKey.SourceTypeValue" placeholder="请选择广告栏位置"
                @change="selector(showKey,showKey.SourceType,showKey.SourceTypeValue)">
                <el-option
            ...
            <li >
              ...
              <el-input
                placeholder="请填写广告内容"
                v-model.trim="selectorBehindObj.remark">
              </el-input>
            </li>
            <li >
              <span class="list_name">  跳转链接<b style="color: #EA5413">*</b> </span>

              <el-input
                placeholder="请填写跳转链接"
                v-model.trim="selectorBehindObj.url">
              </el-input>
            </li>
            <li>
              <span class="list_name demonstration"> 展示周期<b style="color: #EA5413">*</b></span>
                <el-date-picker
                  class="timeinfo"
                  v-model.trim="value3"
                  type="datetimerange"
            ...
            <li >
              ...
              <el-input-number
                :min="1"
                :max="9999"
                v-model.trim="selectorBehindObj.sort">
              </el-input-number>
            </li>
          </ul>
...
//        console.log(this.selectorBehindObj);
        if(!this.selectorBehindObj["type"])return this.$message({message: `请选择推送对象`,});
        if(!this.selectorBehindObj["showKey"]){
          return this.$message({message: `请选择广告栏位置`,});
        }
        if(this.value3.length==0){
          return this.$message({message: `请选择展示周期`,});
...
//              console.log(data,"data");
      ...
      selector(item,values,SourceTypeValue){       //选中后的下拉列表
        ...
        values.forEach((v,i)=>{
          if(v.value===SourceTypeValue){
            this.$set(this.selectorBehindObj,key,v.id)
          }
        })
      },
    ...
    watch:{
      "selectorBehindObj.type":function(o,l){
        this.showKey.SourceType = o === "1" ? this.$store.state.showKeyUser : this.$store.state.showKeyMaster;
      }
    }
  }
</script>
...
<style scoped lang="scss">
  ...
  .master{background-image: url("../../../assets/images/master-banner.png")}
  ul{
    padding-left:10px;
    list-style:none;
    >li{

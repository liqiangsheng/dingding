<template>
    <div id="app">
      <div class="box">
        ...
        <textarea class="alert_forbidden_area" v-model.trim="textareaContent" placeholder="输入其他原因">
        </textarea>
        <div class="bottom">
          <el-button @click="close">取消</el-button>
          <el-button @click="masterImposeOperate(addRemarksObj.orderId)" type="primary">确定</el-button>
        </div>
      </div>
    </div>
</template>
    ...
    export default {
      props:["addRemarksObj","quiry"],
        ...
        methods: {
          ...
          masterImposeOperate(item){
            if(this.textareaContent == ""){
              ...
              this.$http.post(url,{"remark":this.textareaContent,"orderId":item}).then(r=>{
                let data=r.data;
                if(data.code==='0000'){
                  this.$message({
                    message: '提交成功',
        ...
        created() {
          ...
          if(date > this.addRemarksObj.timeOut){
            this.outTime = true;
          }else{
            this.outTime = false;
          }

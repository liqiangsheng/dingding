<template>
  <div>
    ...
    <header v-if="homeShow">
      <div class="operate_button_left" @click="openCarte" v-if="menuShow">
      ...
      <div class="operate_button_right" v-if="menuShow">
   ...
   <header class="header2" v-else-if="masterDetailShow">
     ...
     <div v-if="DisableNub==2" @click="recoveryMaster" style="position: absolute;right: 0.5rem;font-size:0.28rem;font-family:PingFangSC-Regular;color:#fff">
        恢复
     </div>
     <div v-else-if="DisableNub==1"  @click="disableMaster" style="position: absolute;right: 0.5rem;font-size:0.28rem;font-family:PingFangSC-Regular;color:#fff">
       禁用
     </div>
   </header>
...
//            this.$router.go(-1);
          ...
          recoveryMaster(){
            ...
            objData.id = JSON.parse(localStorage.getItem("masterDetail")).id;
            objData.state = "1";
            objData.templateCodeString = "huifu";
...
//                   this.$router.go(-2);
      ...
      computed: {
        ...
        watch:{
           "$route":function(a,b){
            if(a.path==="/login"){
             ...
             this.addMaster = a.fullPath.indexOf("/addMaster") != -1 || a.fullPath.indexOf("/serviceArea") != -1 || a.fullPath.indexOf("/serviceWorker") != -1 || a.fullPath.indexOf('/editMasterDetails') != -1 || a.fullPath.indexOf('/masterPosition') != -1 ? false : true;
             if(a.fullPath === "/masterOrder"||a.fullPath === "/dotInformation"||a.fullPath ==="/messageCenter"||document.title === "工单详情"||a.fullPath ==="/masterPosition"||a.fullPath ==="/masterDisable"){
               ...
               if(JSON.parse(localStorage.getItem("masterDetail")).state === "2"){
                 this.DisableNub = 2;
               }else if(JSON.parse(localStorage.getItem("masterDetail")).state === "1"){
                 this.DisableNub = 1;
               }
               this.masterDetailShow = true;
               this.masterOlderShow = false;
               this.homeShow =false;
             }else if(a.path.includes("/changeMaster")||a.path.includes("/unusualReport")){
        ...
        created() {
          ...
          if (width < 800) {
            //window.location.href="http://site.testdist.dingdingkuaixiu.com"
          } else {
            window.location.href = this.$common.siteWebDomain;
          }
        }
  }
function getMessage(){
  let url =`${this.$common.apidomain}/sysMessage/findPageByUnRead?state=0`;
  this.$http.get(url).then(res => this.$httpFilter(res).then(data=>{
        let result= data.result;
      if(!!result.list&&result.list.length){
          this.sysMseeageOrder=true
        }else{

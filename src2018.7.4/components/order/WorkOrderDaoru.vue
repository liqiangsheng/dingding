<template>
  <div class="OrderDaoru" ref="OrderDaoru">
    <DaoruSueess v-if="daoruShow" @isClose="isClose" @isEmty="isEmty"></DaoruSueess>
    <!--<Upload-->
      <!--ref="upload"-->
      <!--:show-upload-list="false"-->
      <!--:on-success="uploadSuccess"-->
      <!--:on-error="uploadError"-->
      <!--:action="upload" :headers="headers1">-->
      <!--<Button style="color: #FFFFFF;background: #20a0ff;height: 36px;" type="ghost">导入工单</Button>-->
    <!--</Upload>-->
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        Arr:[""],
        listName:["家电清洗","家电维修","家居维修","管道疏通","灯具线路","卫浴洁具","安装服务"],
    ...
    methods: {
      ...
      downModel(item, index) {//下载模板
       if(index == 0){
         window.location =this.$apiModel + "/manageOrderImport/家电清洗.xlsx";
       }else if(index == 1){
         window.location =this.$apiModel + "/manageOrderImport/家电维修.xlsx";
       }else if(index == 2){
         window.location = this.$apiModel + "/manageOrderImport/家居维修.xlsx";
       }else if(index == 3){
         window.location =this.$apiModel + "/manageOrderImport/管道疏通.xlsx";
       }else if(index == 4){
         window.location =this.$apiModel + "/manageOrderImport/灯具线路.xlsx";
       }else if(index == 5){
         window.location =this.$apiModel + "/manageOrderImport/卫浴洁具.xlsx";
       }else if(index == 6){
         window.location =this.$apiModel + "/manageOrderImport/安装服务.xlsx";
       }
      },
      onchangeFile(event) {

        ...
        <table class="tableProduct">
          ...
          <tbody>
           <tr v-for="(item,index) in productList.tableListData" :key="index" :class="{active:item.status == 1}">
             ...
             <td v-if="!item.isflag">{{item.price2Sale|placeholder_one}}</td>
             <td v-if="!item.isflag">{{item.price2Voucher|placeholder_one}}</td>
        ...
        this.$http.get(url,params1).then(r=>{
          ...
          if(data.code==='0000') {
            ...
            this.$http.get(url).then(r=>{
              ...
              console.log( this.dataObj.serveAreas ,'修改服务地址')
            })
          }
        });

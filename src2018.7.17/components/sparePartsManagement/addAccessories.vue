                ...
                <div class="dable_lsit" >
                  ...
                  <table cellSpacing="0px" cellpadding="0"  id="table">
                    ...
                    <tr v-for="(item,index) in tableListData.list">
                      ...
                      <!--工程师信息-->
                      <td>
                        {{item.serviceMountings.coreMainOrder.masterName}}
                        <br>
                        {{item.serviceMountings.coreMainOrder.masterPhoneNum}}
                      </td>
                      <!--申领时间-->
                      ...
                      <!--操作-->
                      <td id="iconS">
                          <span v-if="item.state == '10'||item.state == '50'" @click="okAlter(name='申领通过')">通过</span>
                          <span v-if="item.state == '10'"  style="color: #E65831" @click="okAlter(name='申请驳回')">驳回</span>
                          <span v-if="item.state == '20'" id="deliverGoods" @click="okAlter(name='发货')">发货</span>
                          <span v-if="item.state == '30'"  id="okGoods" @click="okAlter(name='确认取件')">确认取件</span>
                          <span v-if="item.state == '40'"id="endGoods" @click="okAlter(name='完结')">完结</span>
                          <span v-if="item.state == '10'||item.state == '20'||item.state == '30'||item.state == '40'||item.state == '50'||item.state == '60'"  id="detailGoods" @click="detailAlter(name='申领详情',item.id)">详情</span>
                      </td>
  ...
  export default {
    ...
    data() {
      return {
        ...
        theadsName:["序号","配件名称","产品全称","工单号","工程师信息","申领时间","取件方式","是否已返还旧件","申领状态","操作"],
        tableList:{
          pageNo:1,
          pageSize:20,
          total:0,
...
//        this.$store.commit("parseDetailiD",{id:"",isBool:true})

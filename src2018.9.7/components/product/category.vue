    ...
    <div class="container" style="width: 101%">
      <div class="selector_list">
        ...
        <div class="list">
          {{lastLevel.name}} :
          <el-select
            v-model.trim="lastLevel.SourceTypeValue" placeholder="请选择"
            @change="selectorArea(lastLevel,lastLevel.SourceType,lastLevel.SourceTypeValue)">
            <el-option
        ...
        <div class="list">
          <!--一级分类 :-->
          <!--<el-select id="yiiId" v-model.trim="yiji" placeholder="请选择" @change="yijifenlei2">-->
          <!--<el-option-->
          ...
          <tr v-for="(item,index) in tableListData.labelBusinesses">
            <td>
              <el-checkbox v-model.trim="isCheckboxList[index]"
                           @change="isCheckbox(isCheckboxList[index],index)"></el-checkbox>
            </td>
            <td style="text-align: left;text-indent: 10px">
              {{item.id}}

              <input type="text" :value="this.$wechatProductAssortment+duplicateId(item.id)+'/'+item.id.substring(0,3)+'?id='+item.id" class="link">
            </td>
            ...
            <td  @dblclick="editInput(item,isEditTable,index)" @keyup.enter="enter(item,isEditTable,index,isEditTable[index].sublevelTitle)" >
              ...
              <el-input @blur="enter(item,isEditTable,index,isEditTable[index].sublevelTitle)"
                        v-show="!isEditTable[index].isEdit" v-model.trim="isEditTable[index].sublevelTitle"></el-input>
            </td>
            <td>
            ...
            <td class="cursor">
              ...
              <!-- 复制 分类id -->
                ...
                <span>
                  <a target="_blank" :href="this.$wechatProductAssortment+duplicateId(item.id)+'/'+item.id.substring(0,3)+'?id='+item.id">
                 预览</a>
                </span>
            </td>
          </tr>
          ...
          this.$http.get(urlOne).then(res=>{
            if(res.data.code == "0000"){
              async function getData(){
                ...
                for(let i=0;i<this.yijifenlei.length;i++){
                  let qqqqqq = this.$common.apidomain + '/articleinfo/findlabelbusinessbyflabel?id=' + this.yijifenlei[i].id;

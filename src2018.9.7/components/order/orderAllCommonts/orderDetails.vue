        ...
        <div v-if="telescopic">
          ...
          <table class="tableProduct" style="border-top: 1px solid #C0CCDA" v-for="(item,index) in productList.tableListData" :key="index"
            ...
            <tr>
              ...
              <td>
                ...
                <span>￥{{item.price2Original === item.price2Sale ? item.price2Original : item.price2Sale}}</span>
              </td>
              <!--券后服务费-->
              <td>
                <del style="color:#888888;display: block" v-if="item.price2Original !==item.price2Voucher">
                  ￥{{item.price2Original | placeholder_one}}
                </del>
                <span>￥{{item.price2Original === item.price2Voucher ? item.price2Original : item.price2Voucher}}</span>
              </td>
              <td>{{item.serviceSize | placeholder_one}}</td>
              <td>￥{{item.price2 * item.serviceSize | placeholder_one}}</td>
          ...
          <table class="tablePrice" style="border-top: 1px solid #C0CCDA">
            <tr>
              ...
              <td>配件费小计<span >￥{{orderData.price3Original|twoNum}}</span></td>
              <td>改价金额<span>￥{{orderData.changePriceTotal|twoNum}}</span></td>
              <td>工单合计<span>￥{{orderData.originalTotal|twoNum}}</span></td>
            </tr>
            <tr>
              ...
              <td colspan="2">用券折扣<span style="left: 89%">￥{{orderData.discountPrice|twoNum}}</span></td>
...
<script>
  ...
  export default {
    ...
    props: ["orderDetailed", "quiry", 'drawbackIsShow', 'addRemarksIsShow', 'replaceMaster', 'complainFn',
      //工单报备     罚款工程师             改约时间         新增服务产品addSaverProduct        催单 处理
...
//        this.time.sum=sum;
      ...
      isalerShow() {
        ...
        this.$store.dispatch("orderDetailiD", {data: "", isBool: true, title: "全部工单"})
      },
      charge(item) {

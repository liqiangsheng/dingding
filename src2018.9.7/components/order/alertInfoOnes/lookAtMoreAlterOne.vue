      ...
      <div class="lookAtMoreAlter_box">
        ...
        <div id="lookAtMoreAlterBox">
         ...
         <div class="foot">
           ...
           <el-button type="primary" style="width: 140px" @click="add()" :disabled="addDis">添加</el-button>
         </div>
      </div>
  </div>
</template>
...
//      Pagination,
    ...
    props: ["QuDaoId", "numID", "cityId","channelId"],
    data() {
      return {
        addDis:false,//服务是否售罄
        lLindex: -1,
...
//      this.xiaoqudaoid=this.$store.state.xiaoqudaoId?this.$store.state.xiaoqudaoId:JSON.parse(sessionStorage.getItem("userInfo"))[0].id
    ...
    methods: {
      fenlei() {
        // 分类
        let url = this.$apidomain + "/common/findflabelbusinessname?channelId="+this.channelId;
...
//        let ControllerUrl = `${this.$apidomain}/officialProductLabelController/all?serviceId=${id}`;
      ...
      deleteHistory(v,i){ //删除
        let detailUrl = `${this.$apidomain}/officialProductLabelController/delete?serviceId=${v.serviceId}&serviceBrand=${v.serviceBrand ? v.serviceBrand : ""}&serviceModel=${v.serviceModel ? v.serviceModel : ""}&type=1&linkmanAreaId=${v.linkmanAreaId}`;

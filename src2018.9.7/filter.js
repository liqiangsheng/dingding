...
Vue.filter('refundStateShow',function(input){
  if("Pay"==input){
    return "已退款";
  }else if("Refund"==input | 'WaitingForRefund'==input){
    return "待退款";
  }else if(input=='RefundFailed'){
    return '退款失败'
  }
  return "无"
});
...
Vue.filter('advertisingShow',function(input){
  switch (input){
    ...
    case "SystemNoticeByMaster":return "工程师系统公告";
    case "PopupHeadByMaster":return "工程师首页弹窗";
    case "HomePageHeadByMaster":return "工程师首页头部banner";
  }
  return "无"
});
...
Vue.filter('stateShow',function(input){
  if("1"===input){
    return "已上架";
  }else if("0"===input){
    return "未上架";
...
Vue.filter('payTypeShow',function(input){
  if("Reappearance"==input){
    return "返现";
  }else if("RefundPortion"==input||'RefundAll'==input||'Refund'==input){
    return "退款";
  }else if("MasterTurn"==input){
    return "上缴";
  }else if("Pay"==input){
    return "支付";
  }else if("UserCode" == input){
    return '用戶扫码支付'
  }else{
    return '支付'
  }
...
Vue.filter('serverType',function(input){
  if("1"==input){
    return "小修";
  }else if("2"==input){
    return "中修";
...
Vue.filter('parStates',function(input){
  if(input==='Pay'||input==='Refund'||input==='Refunded'){
    return "已退款"
  }else if(input==='WaitingForRefund'){
    return "待退款"
  }else{
    return "退款失败"
  }
...
Vue.filter('examineState',function(input){
  if(input===0){
    return '待审核'
  }else if(input===1){
    return '审核通过';
...
Vue.filter('twoNum',function(value){
  if(!value){
    return 0
  }else{

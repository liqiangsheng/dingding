<template>
  <div id="app">
      ...
      <div class="selector_list">
        ...
        <div  v-for="(item,index) in optionList" :key="index" class="list">
          ...
          <el-select
            ...
            @change="selectorTen(item,item.SourceType,item.SourceTypeValue)">
            <el-option
      ...
      <div class="dable_lsit" >
        ...
        <table cellSpacing="0px" cellpadding="0"  id="table">
          ...
          <tr v-for="(item,index) in tableListData.orders">
            ...
            <!--工单号-->
            <td @click="orderDetails(item,item.id)" id="orderId" style="word-break: break-all;">
              <img v-show="item.emergencyDegree == 0" src="/static/images/jiaji.png" style="display: inline-block;width: 22px;height: 22px;transform: translateY(5px)">
              {{item.id|placeholder}}
            </td>
            <!--工单类型-->
             <td>
            ...
            <td>
              ...
              <!--{{item.officialPartnerName?item.officialPartnerName+'-'+item.officialPartnerSubsetName:"无"}}-->
            </td>
            <!--渠道质保-->
            <td>
            ...
            <td class="icon" id="iconS">
              ...
              <!--详情-->
              <ul class="iconShow" v-show="item.iconShow" @mouseleave="outHide(item,index)">
                <li>
                  <Tooltip content="派单"
                           placement="top-end"
                           v-if="sendOrder(item.state)"
                ...
                <li>
                  <Tooltip content="添加备注" placement="top-end" hidden v-show="$isButtonObj('bn-order-all-remark')">
                    <span  @click="addRemarksIsShow(item)">添加备注</span>
                  </Tooltip>
                </li>
                <li>
    ...
    <!--异常挂起 end-->
    <!--退款-->
      <drawback :isDrawback="isDrawback" :data="drawbackData" v-if="isDrawback.isShow" @isClosed="isClosed">
        ...
        optionList: [       //下拉列表数据
          {
            name: "工单状态",
            key: "orderStatus",
            SourceTypeValue: '', //选中后的"04,05,06,07,10,11,13",
            SourceType:[{value:"待上工",id:"04"},{value:"待抵达",id:"05"},{value:"待分配",id:"06"},{value:"服务中",id:"07"},{value:"待核账",id:"10"},{value:"待付尾款",id:"11"},{value:"待退款",id:"13"}]
          }
        ],
        stateOne:"04,05,06,07,10,11,13",
        selectorBehindObj: {},
        theadsName: [
          '序号',
          '工单号',
...
//          '下单手机号',
        ...
        sortData: [
          ...
          {
            index:8,
            sortAscendingValues:[" master_name asc "],  //升序
            gradeDownValues:[" master_name desc "],     //降序
          },
          {
            index:6,
            sortAscendingValues:[" linkman_phone_num asc "],  //升序
            gradeDownValues:[" linkman_phone_num desc "],     //降序
          }
        ],
        tableListData: {},          //表格数据
        showPages: 1,
...
//            添加师傅详情结束
    ...
    created(){
      ...
      this.labeloptions=[{name:'--请选择--',id:''},...this.$store.state.findchannel]

     // console.log(this.labeloptions[0],'3333333');
      this.labeloptions.forEach((obj,i)=>{
        this.$set(obj,'childChannel',[])
      })
//      console.log(this.$store.state.findchannel,'999999999')
      //获取城市 start
...
//      this.getData({optionLists:this.optionList,key:"sourceId",data:this.$store.state.findchannel,resName:"name"})
    ...
    methods: {
      selectorTen(v,i,item){//工单状态
        this.optionList[0].SourceType.forEach((v1,i1)=>{
          if(item == v1.value){
            this.stateOne = v1.id;
          }
        })
      ...
      reset(){//重置
        location.reload();
//        this.$route.push({path:"/order/list5"})
      },
      jiantouClick(){ //搜索消失出现

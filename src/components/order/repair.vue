<template>
  ...
  <div id="app">
    ...
    <div v-show="orderDetailed.isShow" class="container">
      ...
      <!--查询按钮结束-->
      <el-button v-for="(item,index) in returnRepairBtn" :key="index" @click="returnRepair(index)" size="large" class="derive_btn" v-show="$isButtonObj('bn-order-all-new')">{{item.name}}</el-button>
      ...
      paramsData(){
        ...
        if(!!this.selectorBehindObj.accomplishDate&&this.selectorBehindObj.accomplishDate.length){
           accomplishDate = this.selectorBehindObj.accomplishDate.map( v =>{
            if(this.$moment(v).format("YYYY-MM-DD").includes("Invalid")){
             return ""
            } else {
              return this.$moment(v).format("YYYY-MM-DD")
            }
          }
          );
        }

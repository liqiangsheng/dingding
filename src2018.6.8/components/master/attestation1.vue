<template>
  <div id="app">
    <div class="container" style="width: 101%">
      <div class="selector_list">
        ...
        <div class="list">
          认证来源:
          <el-select
            v-model="laiyuan" placeholder="请选择" @change="laiyuanType(laiyuan)">
            <el-option
      ...
      <div class="dable_lsit">
        ...
        <table cellSpacing="0px" cellpadding="0">
          ...
          <tr v-for="(item,index) in tableListData.masterInfos">
            ...
            <!--操作-->
            <td>
                <span class="recover" v-if="'2'!=item.certificateState"
                      v-show="$isButtonObj('bn-master-authentication-details')">
                  <el-button type="text" @click="open2(item,false)"><i></i>
                    {{text.forbidden}}
                  </el-button>
                </span>
              <span :class="{'recover':item.btnType=='0'}" v-if="'2'!=item.certificateState"
                    v-show="$isButtonObj('bn-master-authentication-details')">
                  <el-button type="text" @click="open2(item,true)"><i></i>
                  {{text.recover}}
                </el-button></span>
              <span class="details_icon" v-show="$isButtonObj('bn-master-authentication-details')"><el-button
                type="text" @click="details(item,index)"><i></i>详情</el-button></span>
            </td>
          </tr>
          </tbody>
        </table>
    ...
    <el-dialog style="z-index:999999" :visible.sync="dialogTableVisible" size="tiny">
      ...
      <el-button class="confirm_btn" @click="confirmForbidden(alertForbiddenList,textareaContent)" type="primary">确定
      </el-button>
    </el-dialog>
  </div>
</template>
  ...
  export default {
    ...
    data() {
      return {
        ...
        selectorBehindObj: {"certificateState": "0"},
        tableListData: {
          pageNo: 1,
          pageSize: 20,
          total: 0,

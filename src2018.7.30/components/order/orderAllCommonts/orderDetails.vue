  ...
  <div id="app" v-cloak @touchmove.prevent>
    ...
    <div class="headerBtn bottom">
      ...
      <div>
        ...
        <p class="cursor"
           v-if="!timeIsIf&&changeTime(orderData.state)"
           @click="yuYueTimeIsShow(orderData)">
          预约时间
        </p>
             ...
             <td colspan="14" v-if="item.isflag" class="onlyClass">
               <ul>
                 <li  v-for="(todo,index) in item.mountings" :key="index">
                  <span> 配件{{index+1}}: &nbsp {{todo.serviceMountingsName}}*{{todo.serviceMountingsCount}} </span>
      ...
      findOne(){
        ...
        this.$http.post(url2,{'id':this.orderDetailed.orderId}).then(r=>{
          if(r.data.code=="0000"){
            ...
            console.log(this.repaireorder.list,'返修信息')
          }else{
            alert(r.data.error)
          }
        }).catch(e=>{
        ...
        this.$http.get(url,params1).then(r=>{
          ...
          if(data.code==='0000') {
            ...
            console.log(this.productList.tableListData,'产品信息')

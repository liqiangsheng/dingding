<template>
  <el-row class="container">
    ...
    <div class="info"v-if="!isChannel">

      <b :class="{unread:isUnread===true}" @click="infoLink" ></b>
      <b class="cityIndex"> {{cityNameLi}}</b>
      <b class="unread secede" @click="secede">退出</b>
      <!--<span  @click="biaoqin" style="width: 150px;display: inline-block;transform: translateX(-50px)">打开新页签</span>-->
    </div>
...
<!--渠道 nav end-->
    ...
    <el-col :span="24" class="main" :class="{'channl_aside':isChannel}">
      ...
      <aside :class="!collapsed?'showSidebar':'shrink'"  class="left_nav" :style="isChannel?'margin-top:80px;background:#fff':''">
        ...
        <div class="menu-toggle" @click.prevent="collapse" v-if="!isChannel" style="position:relative">
          <span class="user_name">{{userName}}</span>
          <i class="iconfont icon-menufold cursor" v-show="!collapsed"
             style="position: absolute;left: 70%;top: 40%;"></i>
          <i class="iconfont icon-menuunfold cursor" v-show="collapsed"
             style="position: absolute;left: 10%;top: 20%;"></i>
        </div>

        <div class="menu-toggle_channel" v-else style="position:relative">
          ...
          <i class="iconfont icon-menufold cursor" v-show="!collapsed" @click.prevent="collapse"
             style="position: absolute;left: 70%;top: 40%;"></i>
...
<!--左侧导航 start 后台 -->
        <el-menu default-active="0" router :collapse="collapsed" v-if="isBack" >
...
<!--左侧导航 end 后台 -->
        ...
        <!--左侧导航 start 网点 -->
        <el-menu default-active="0" router :collapse="collapsed" v-if="isSite" >
          <template v-for="(item,index) in $router.options.routes" v-if="item.menuShow">
          <!--<template v-for="(item,index) in routerList" v-if="item.menuShow">-->
            <el-submenu v-if="!item.leaf" :index="index+''">
              <template slot="title"><i :class="item.iconCls"></i><span slot="title">{{item.name}}</span></template>
 ...
 <!--左侧导航 渠道端 start-->
        <!--<el-menu default-active="0" router :collapse="collapsed" v-else id="menu">-->
          <el-menu default-active="0" router :collapse="collapsed" v-if="isChannel" id="menu">
          <template v-for="(item,index) in $router.options.routes" v-if="item.menuShow">
            <el-submenu v-if="!item.leaf" :index="index+''">
 ...
 <!--左侧导航 渠道端 end-->
      ...
      <section class="content-container" :class="{'content-container_channel':isChannel}">
        <div id="qipao" v-if="isShowHelp=='1'&&helpName&&helpName!=''">
          <header><span></span></header>
          <h4><img src="/static/images/helpWT.png" alt=""><span>问题求助</span></h4>
          <p>您有一条工单尾号为{{helpName}}的问题求助</p>
        </div>
<!--tab 网点后台标签栏start-->
...
//  console.log(window,'window0000')
  ...
  export default {
    ...
    name: 'home',
    created(){
         this.cityNameLi = JSON.parse(localStorage.getItem("cityValue"));
         setInterval(()=>{ //工单求助定时任务
           let  url = `${this.$apidomain}/manage/order/findUntreatedAskNow`
           this.$http.post(url).then(res=> {
             if(res.data.code==="0000"){
               if(res.data.result&&res.data.result.length>0){
                 this.helpName = res.data.result[0].mainOrderId.substr(res.data.result[0].mainOrderId.length - 4);
                 if (this.helpName || this.helpName != "") {
                   setTimeout(() => {
                     this.helpName = "";
                   }, 4000)
                 } else {
...
//        }
//      })
//      let url2 =`${this.$apidomain}/sysMessage/findredismessage`;4343
...
//      })
    ...
    data () {
      return {
        helpName:"",
        isShowHelp:getSession("KEY")[0],
        cityNameLi:"", //城市名字
        routerPath:this.$store.state.routerPath.fullPath,
        isBack:getSession("KEY")[0]==="1",   //判断是否是后台
        isSite:getSession("KEY")[0]==="2",   //判断是否是网点
...
//          },
        ...
        userName:JSON.parse(localStorage.getItem("enrolleeinfo")).fullName,
        size:0,
        OrderDataList:[],
        inde:0,
        isUnread:true,
...
//      },
        getRouter(){
          let url = this.$role_api_host +  '/sys/menu/menus?userId=' + JSON.parse(localStorage.getItem("enrolleeinfo")).id
...
//                  this.tabPathList.push(o)
        ...
        //退出登录
          secede(){
            ...
            removerSession("userInfo");
            removerSession("access-user");
            localStorage.removeItem("enrolleeinfo");
      ...
      customer_service_link(){  //给token去请求免登录的客服系统
        let _this=this;
        _this.$http.get(`${_this.$apidomain}/udesk/usersAuthJwtCallback?token=`+JSON.parse(localStorage.getItem("enrolleeinfo")).token).then(function(res){
          if(res.data.code==='0000'){
...
/*渠道端 tabs start*/
 ...
 /*渠道端 tabs start*/
  ...
  .spare_icon{
    background:url(../../static/images/spare.png)center center no-repeat;
    background-size:100% 100%;
  }
  .yunying_icon{
...
#qipao{
  ...
  header{
    ...
    span:hover{
      background: url(../../static/images/pruduct1close.png) no-repeat center center;
    }
  }
}

        ...
        <div class="left_info">
          ...
          <ul class="add_reminded">
            <li style="padding-left: 20px;height: 40px">
              <span class="list_name">{{parseType.name}}</span><span style="margin-right: 3px">:</span>
             {{this.edit.item.masterType|mastertype}}
            </li>
            <li style="padding-left: 20px;height: 40px">
              <span class="list_name">{{parseLeave.name}}</span><span style="margin-right: 3px">:</span>
              {{this.edit.item.masterLevel|masterLevel}}
            </li>
            <li style="padding-left: 20px;height: 40px">
              <span class="list_name">{{cityArray.name}}</span><span style="margin-right: 3px">:</span>
             {{this.edit.item.area}}
            </li>
            <li id="listName1" style="padding-left: 20px;height: 40px">
              <span class="list_name">服务工种</span><span style="margin-right: 3px">:</span>
              {{this.edit.item.labelName}}
            </li>
            <li style="padding-left: 20px;height: 40px">
              <span class="list_name">服务类型</span><span style="margin-right: 3px">:</span>
              {{this.edit.item.serviceType}}
            </li>
          </ul>
        </div>
        <div id="xian1"></div>
    ...
    watch: {
      RemindedList:{
        handler:function(newData,oldData){
           if(newData){
             newData[0].SourceType.forEach((item,index)=>{
               if(item.danweiStr == "%"){
                 item.maxNum = 100;
               }else if(item.danweiStr == "元/单"){
                 item.maxNum = 1000;
               }
             })
           }
...
//          shareValue:this.edit.item.shareValue+"",
          serviceType:this.edit.item.serviceType,
        }}
                ...
                // this.RemindedList[0].SourceType[0].shareTypeKey = v1.shareTypeKey;
                 ...
                 this.RemindedList[0].SourceType[0].danwei.forEach((v,i)=>{
                      if(v.danweiid === this.RemindedList[0].SourceType[0].shareTypeKey){
                        ...
                        this.RemindedList[0].SourceType[0].shareTypeKey = v.danweiid;
                      }
                    })
               }
               console.log(this.RemindedList[0].SourceType[0])
    ...
    methods: {
      Preservation(){ //保存
            if(!this.edit.item){
              if(this.parseType.SourceTypeValue == ''){
                this.parseType.isBool ="请选择工程师类型"
                return
              }else{
                this.parseType.isBool = "";
              }
              if(this.parseLeave.SourceTypeValue == ''){
                this.parseLeave.isBool ="请选择工程师等级"
                return
              }else{
             ...
             this.selectorBehindObj.id = this.edit.item.id;
              ...
              this.selectorBehindObj.configIdsJson  =  aArray.length>0?JSON.stringify(aArray):"";
              this.selectorBehindObj.serviceType =  this.edit.item.serviceType; //服务类型
      ...
      selector(item,v,i){//工程师类别 //工程师等级 //服务城市
        if(item.name === "工程师类别"){
          if(v){
            this.parseType.isBool = "";
            v.forEach((vI,index)=>{
        ...
        if(item.name === " 服务类型"){
          if(v){
            v.forEach((vI,index)=>{
              if(vI.value === i){
                this.selectorBehindObj.serviceType = vI.value;
              }
            })
          }
        }
      ...
      selectorOne1(v, i) { //单位选择
        if(v.feeType === "上门费"){
          this.RemindedList[0].SourceType[0].danweiStr = v.danweiStr
          if(v.danweiStr == "元/单"){
            this.RemindedList[0].SourceType[0].maxNum =1000;
            this.RemindedList[0].SourceType[0].shareTypeKey = "Quota";
          }else if(v.danweiStr == "%"){
            this.RemindedList[0].SourceType[0].maxNum =100;
            this.RemindedList[0].SourceType[0].shareTypeKey = "Proportion";
          }
          if(this.RemindedList[0].SourceType[0].shareValue >this.RemindedList[0].SourceType[0].maxNum){
            this.RemindedList[0].SourceType[0].shareValue = 0;
          }else{
            this.RemindedList[0].SourceType[0].shareValue = this.RemindedList[0].SourceType[0].shareValue;
          }
        }
        if(v.feeType === "服务费"){

          this.RemindedList[0].SourceType[1].danweiStr = v.danweiStr
          if(v.danweiStr == "元/单"){
            this.RemindedList[0].SourceType[1].maxNum =1000;
            this.RemindedList[0].SourceType[1].shareTypeKey = "Quota";
          }else if(v.danweiStr == "%"){
            this.RemindedList[0].SourceType[1].maxNum =100;
            this.RemindedList[0].SourceType[1].shareTypeKey = "Proportion";
          }
          if(this.RemindedList[0].SourceType[1].shareValue >this.RemindedList[0].SourceType[1].maxNum){
            this.RemindedList[0].SourceType[1].shareValue = 0;
          }else{
            this.RemindedList[0].SourceType[1].shareValue = this.RemindedList[0].SourceType[1].shareValue;
          }
        }
        if(v.feeType === "配件费"){
          this.RemindedList[0].SourceType[2].danweiStr = v.danweiStr
          if(v.danweiStr == "元/单"){
            this.RemindedList[0].SourceType[2].maxNum =1000;
            this.RemindedList[0].SourceType[2].shareTypeKey = "Quota";
          }else if(v.danweiStr == "%"){
            this.RemindedList[0].SourceType[2].maxNum =100;
            this.RemindedList[0].SourceType[2].shareTypeKey = "Proportion";
          }
          if(this.RemindedList[0].SourceType[2].shareValue >this.RemindedList[0].SourceType[2].maxNum){
            this.RemindedList[0].SourceType[2].shareValue = 0;
          }else{
            this.RemindedList[0].SourceType[2].shareValue = this.RemindedList[0].SourceType[2].shareValue;
          }
        }
        if(v.feeType === "高空费"){
          this.RemindedList[0].SourceType[3].danweiStr = v.danweiStr
          if(v.danweiStr == "元/单"){
            this.RemindedList[0].SourceType[3].maxNum =1000;
            this.RemindedList[0].SourceType[3].shareTypeKey = "Quota";
          }else if(v.danweiStr == "%"){
            this.RemindedList[0].SourceType[3].maxNum =100;
            this.RemindedList[0].SourceType[3].shareTypeKey = "Proportion";
          }
          if(this.RemindedList[0].SourceType[3].shareValue >this.RemindedList[0].SourceType[3].maxNum){
            this.RemindedList[0].SourceType[3].shareValue = 0;
          }else{
            this.RemindedList[0].SourceType[3].shareValue = this.RemindedList[0].SourceType[3].shareValue;
          }
        }
      },
      changeCount1(v){//失去焦点

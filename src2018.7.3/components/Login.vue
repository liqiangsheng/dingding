<template>
  <el-form v-show="isChannel" ref="AccountFrom" :model="account" :rules="rules" label-position="left" label-width="0px"
    ...
    <!--<el-checkbox v-model.trim="checked"  checked class="remember">记住密码</el-checkbox>-->
...
//              Cookies.set('token', data.result, { expires: 7, path: '/' })
...
//        Cookies.set('token', getLocalStorageUserInfo("enrolleeinfo")[0], { expires: 7, path: '/' });
        this.$router.push({ path: '/'});
      }
      this.$router.push({ path: '/' });
    }

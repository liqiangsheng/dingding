      ...
      <div class="scrollbar content">
        ...
        <section class="container_content">
            <aside class="container_content_left">
              ...
              <!--<div>-->
                <!--<inputs-list :item="inputs[0]" v-if="isInput(isEdit,inputs[0].isEdit)"></inputs-list>-->
                <!--<div class="list_name" style="width:93px;" v-if="!isInput(isEdit,inputs[0].isEdit)"> {{inputs[0].name}}<span v-show="siteOne" style="color: red">{{siteOne}}</span></div>-->
                <!--<span style="text-indent: 1em;display: inline-block" v-if="!isInput(isEdit,inputs[0].isEdit)">-->
                     <!--{{inputs[0].value}}-->
                <!--</span>-->

              <!--</div>-->
              <!--<div>-->
                <!--<inputs-list :item="inputs[1]" v-if="isInput(isEdit,inputs[1].isEdit)"></inputs-list>-->
                <!--<div class="list_name" style="width:93px;" v-if="!isInput(isEdit,inputs[1].isEdit)"> {{inputs[1].name}}</div>-->
                <!--<span style="text-indent: 1em;display: inline-block" v-if="!isInput(isEdit,inputs[1].isEdit)">-->
                     <!--{{inputs[1].value}}-->
                <!--</span>-->
              <!--</div>-->
              <!--<div>-->
                <!--<inputs-list :item="inputs[2]" v-if="isInput(isEdit,inputs[2].isEdit)"></inputs-list>-->
                <!--<div class="list_name" style="width:93px;" v-if="!isInput(isEdit,inputs[2].isEdit)"> {{inputs[2].name}}</div>-->
                <!--<span style="text-indent: 1em;display: inline-block" v-if="!isInput(isEdit,inputs[2].isEdit)">-->
                     <!--{{inputs[2].value}}-->
                <!--</span>-->
              <!--</div>-->
              <!--<div>-->
                <!--<inputs-list :item="inputs[3]" v-if="isInput(isEdit,inputs[3].isEdit)"></inputs-list>-->
                <!--<div class="list_name" style="width:93px;" v-if="!isInput(isEdit,inputs[3].isEdit)"> {{inputs[3].name}}</div>-->
                <!--<span style="text-indent: 1em;display: inline-block" v-if="!isInput(isEdit,inputs[3].isEdit)">-->
                     <!--{{inputs[3].value}}-->
                <!--</span>-->
              <!--</div>-->
              <!--<div>-->
                <!--<inputs-list :item="inputs[4]" v-if="isInput(isEdit,inputs[4].isEdit)"></inputs-list>-->
                <!--<div class="list_name" style="width:93px;" v-if="!isInput(isEdit,inputs[4].isEdit)"> {{inputs[4].name}}</div>-->
                <!--<span style="text-indent: 1em;display: inline-block" v-if="!isInput(isEdit,inputs[4].isEdit)">-->
                     <!--{{inputs[4].value}}-->
                <!--</span>-->
              <!--</div>-->
        ...
        <section>
          <div class="input_title">
            服务区域<b style="color: #EA5413">*</b><span v-show="cityOne" style="color: red">{{cityOne}}</span>
            <span v-show="areaIdsOne" style="color: red">{{areaIdsOne}}</span>
          </div>
        ...
        <section>
          <div class="input_title">
            服务工种<b style="color: #EA5413">*</b>  <span style="color: red" v-show="skillIdsOne">{{skillIdsOne}}</span>
          </div>
          ...
          <selector-teer v-if="!isShowSkills()&&!!skillList" @skillIdsOne1="skillIdsOne1"
          :list="skillList"
...
* */
    ...
    watch:{
      skillIdsOne2:function(new1,old1){
        if(new1){
          this.skillIdsOne = "";
        }else{
          this.skillIdsOne = "请选择服务主营业务";
        }
      },
      cityOne1:function(new1,old1){
        if(new1){
          this.cityOne = "";
        }else{
          this.cityOne = "请选择城市";
      ...
      areaIdsOne1:function(new1,old1){
        if(new1){
          this.areaIdsOne = "";
        }else{
          this.areaIdsOne = "请选择服务区域  ";
        }
      },
      inputs:{
        handler:function(new1,old1){
          if(new1[0].value != ""){
            ...
            this.inputs[0].isBool = "请填写网点名称，长度不超过20字符，不可输入标点符号"
          }
          if(new1[1].value != ""){
            this.inputs[1].isBool = ""
          }else{
            this.inputs[1].isBool = "请填用户名"
          }
          if(new1[3].value != ""){
          ...
          if(new1[4].value != ""){
            if(new1[4].value.length == 11&&letTest.test(new1[4].value)){
              this.inputs[4].isBool = "";
            }else{
              this.inputs[4].isBool = "请填写正确手机号，长度为11位数字"
            }
            console.log(this.inputs[4].isBool)
          }else{
            this.inputs[4].isBool = "请填写手机号"
          }
        },
...
//        })
      ...
      submitAdd(isEdit=false){
        console.log(this.areaSelects,'123131')
        let params=[];
        const getIds=obj=>{
          let params=[];
        ...
        if(this.selectorBehindObj.username){
          this.inputs[1].isBool = ""
        }else{
          this.inputs[1].isBool = "请填写用户名"
        }
        if(this.selectorBehindObj.linkmanName){
          this.inputs[3].isBool = ""
        }else{
          this.inputs[3].isBool = "请填写联系人"
        }
        if(this.selectorBehindObj.linkmanPhoneNum){
          this.inputs[4].isBool = "";
        }else{
          this.inputs[4].isBool = "请填写手机号,长度为11纯数字"
        }
        if(this.selectorBehindObj.city){
          this.cityOne = "";
        }else{
          this.cityOne = "请选择城市"
        }

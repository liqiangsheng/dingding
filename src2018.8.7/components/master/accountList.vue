      ...
      <div class="container" style="width: 101%;">
        ...
        <div class="dable_lsit">
          ...
          <div class="derive">
            ...
            <el-button @click="derivation(tableListData.masterInfos)" size="large" v-show="$isButtonObj('bn-master-account-newaccount')">导出</el-button>
          </div>
          <table cellSpacing="0px" cellpadding="0">
            <thead>
            <tr class="theads">
              <!--<th>-->
              <!--<el-checkbox v-model="checked" @change="wholeSelector(isCheckboxList,checked)"></el-checkbox>-->
              <!--</th>-->
...
//          unmber:"",//工程师编号
          ...
          sortData: [
            ...
            {
              index:8,
              sortAscendingValues:[" work_state asc "],  //升序
              gradeDownValues:[" work_state desc "],     //降序
            },
            {
              index:6,
      ...
      created(){
        ...
        this.$http.get(urlCount).then(r=>{
          ...
          this.userSum.forEach((v)=>{
            ...
            }else if("总工程师数量"==v.name){
              v.sum = data.result.totalNum;
            }else if("待审核工程师数量"==v.name){
...
//          this.city.SourceType.push({"id":"","value":"---请选择---"});
      ...
      methods: {
        derivation(data){
          const url =`${this.$apidomain}/masterinfo/createMasterInfoExcel`
...
//            "id":this.unmber,
        ...
        getTableList(params){
          ...
          this.$http.get(url,params).then(r=>{
            ...
            this.isCheckboxList = this.tableListData.masterInfos.map( v=> {
             return false;
            }
          );
...
//      <!--弹窗功能start-->
        details(item,index){
          this.isAdd.id = item.id;
          this.isAdd.title= "工程师详情"
...
//      <!--禁用启用操作 end-->
        isCheckbox(item,i){
          let isCheck = true;
          for(let i = 0; i < this.isCheckboxList.length; i++){
        ...
        wholeSelector(list,currentState){
          list.forEach((v,i)=>{ this.$set(this.isCheckboxList,i,currentState) })
        },
        Reminded(item,index){

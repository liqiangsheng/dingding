...
<script>
    ...
    export default{
       ...
       created(){
         //console.log(this.channelList,'000000000000')
         this.objectType = this.channelList;
         ...
         changeSelector2(value){
           ...
           //全选,反选start
            ...
            jump(state){
                ...
                let path=`/finance/billingDetails?id=${state.id}`;
                if(isPushPath(this.$store.state.tabPathList, path))this.$store.commit("pushTabPathList",{
                            path:`/finance/billingDetails?id=${state.id}`,
                            name:"账单明细"
                        });

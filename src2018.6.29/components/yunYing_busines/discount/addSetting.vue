      ...
      <div class="scrollbar content">
       ...
       <ul>
          <li class="add_master">
            <li>
              ...
              <el-input v-if="!isAdd.data"
                        placeholder="请填写内容"
                        v-model.trim="selectorBehindObj.discountName">
              </el-input>
              <span v-else>{{isAdd.data.discountName}}</span>
            </li>
            <li>
              <div class="list_name">有效期 :</div>
              <DatePicker v-if="!isAdd.data" v-model.trim="date" type="datetimerange" format="yyyy-MM-dd HH:mm"
            ...
            <li  v-for="(item,index) in optionList" v-if="isEdit">
              ...
              <el-checkbox style="transform:translateX(20px);" v-model.trim="checked">全部通用</el-checkbox>
            </li>
         <li class="fenle">
         ...
         <li>
           ...
           <el-checkbox v-model.trim="isSuperposition">折扣活动叠加</el-checkbox>
         </li>
            <li>
              <div class="list_name">城市 :</div>
                <select-option style="display: inline-block"
                               :messageList="$store.state.cityData"
                               placeholder="请选择城市"
                               :value="selectorBehindObj.areaId"
                               @selectorData="getCity">
                </select-option>
            </li>
          </ul>
        </div>
  ...
  export default {
    ...
    created(){
        ...
        if(this.isAdd.data){
          ...
          this.selectorBehindObj.areaId=this.isAdd.data.areaId
          this.selectorBehindObj.areaName=this.isAdd.data.areaName
    ...
    methods: {
      getCity(v){
        ...
        this.selectorBehindObj.areaName=v.name;
//        this.cityId
      },
      filter(input){
        if("FA"===input){
      ...
      addData(){
        ...
        let regEn = new RegExp("[`~!@#$^&*()=|{}':;',\\[\\].<>/?~！@#￥……&*（）——|{}【】‘；：”“'。，、？]");

      if(regEn.test(this.selectorBehindObj.discountName)){
        return this.$queryFun.successAlert.call(this,"优惠券名称不得出现特殊字符")
      }
        ...
        if(!this.selectorBehindObj["discountName"]){
          return this.$queryFun.successAlert.call(this,"请填写正确的优惠券名称，不得超过26个字符","0")

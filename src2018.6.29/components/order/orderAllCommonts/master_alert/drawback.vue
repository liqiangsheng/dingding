  ...
  <div id="app">
    ...
    <!--<el-dialog style="z-index:999999"  :visible.sync="isDrawback.isShow" size="tiny">-->
      ...
      <div class="contenter">
        ...
        <div class="netList">
          <table>
            ...
            <tr v-for="(item,index) in tdList" :key="index">
              <td>{{item.name}}</td>
              <td>￥{{item.netValue}}</td>
              <td><input type="text"   v-model.tirm="item.refund" placeholder="￥0">
              </td>
            </tr>
            </tbody>
          </table>
        ...
        <div class="reasons">
          <p>退款原因</p>
          <p v-for="(item,index) in drawback.alertForbiddenList" :key="index" @click="masterImposeSelector(item)">
            <span >
              <img :class="{i:true,isSelect:true,selected:item.selector!='0'}">
            </span>
            <font>{{item.name}}</font>
          </p>
...
<script>
  export default {
    ...
    methods: {
      ...
      getTableList(){
        let url = this.$apidomain+"/order/getorder?mainOrderId="+this.isDrawback.orderId;
        this.$http.get(url).then(res=>{
          let data = res.data;
          if(data.code==='0000'){
            this.totalPay = data.result.serviceTotalPay;
            this.getDetails = data.result;
            console.log(this.totalPay,'333333')
      ...
      masterImposeOperate(item,isDrawback){
        ...
        //console.log(this.totalPrice,'12312')
        ...
        let partParams={"orderId":order,"refund":this.value,"refundReason":`${selector}-${content}`,'refundFee':this.totalPrice};
        let wholeParams = {"orderId":order,"refund":this.value,"refundReason":`${selector}-${content}`};

<template>
  <div>
    <div class="center">
      ...
      <ul>
        <li>产品名称：
          <el-input v-model.trim="selectorBehindObj.fullName" placeholder="请输入产品名称"
                    style="width:200px;margin-left: 7px"></el-input>
        </li>
        <li>配件名称：
          <el-input v-model.trim="selectorBehindObj.name" placeholder="请输入配件名称"
                    style="width:200px;margin-left: 7px"></el-input>
        </li>
      </ul>
      <div class="bnt">
...
//          "检测费",
    ...
    methods:{
      ...
      getTableList(params){ //搜索
        let url=`${this.$apidomain}/serviceinfo/findOneAreaInfoList`;
        this.$http.get(url,params).then(r=>{
          let data=r.data;
          if (data.code == "0000") {
            this.tableListData = data.result;
            if (this.tableListData.serviceAreaMountings.length == 0) {
              this.tableListData.serviceAreaMountings = [{
                areaId: "0101",
                areaName: "深圳",
                id: "无",
                labelId: "暂无配件",

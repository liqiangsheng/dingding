export default {
  ...
  getAddress(e){
    if(!!e.value){
      this.$jsonp(`http://apis.map.qq.com/ws/place/v1/suggestion?region=${!!e.city?e.city:""}&key=NIGBZ-2JCWF-P3HJT-NTO5F-HJX75-GWFDD&output=jsonp&keyword=${e.value}`).then(res=>{
        if(!!res.data){
          ...
          this.$jsonp(url).then(res=>{
            ...
            if(data.infocode==='10000'&&data.info==="OK"){
              ...
              // this.addressData.addressComponent=data.regeocode.addressComponent;

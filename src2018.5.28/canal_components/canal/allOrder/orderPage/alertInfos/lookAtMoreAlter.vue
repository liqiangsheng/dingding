...
//        let ControllerUrl = `${this.$apidomain}/officialProductLabelController/all?serviceId=${id}`;
      ...
      deleteHistory(v,i){
        let detailUrl = `${this.$apidomain}/officialProductLabelController/delete?serviceId=${v.serviceId}&serviceBrand=${v.serviceBrand}&serviceModel=${v.serviceModel}&type=3`;
      ...
      add(){//添加
        ...
        this.$emit("flaga",1);
        let urlThree=this.$common.apidomain+"/articleinfo/findChannelListServiceInfo?labelId="+this.addData.serviceId +"&areaId="+this.$store.state.cityIdid+"&relevanceKey="+this.xiaoqudaoid;

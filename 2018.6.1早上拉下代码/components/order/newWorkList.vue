  ...
  <div id="app">
    <div >
      ...
      <LookAtMoreAlter @isClose="isClose" v-if="lookAtMoreShow" @pejianzengjia="pejianzengjia" @flaga="flaga"
                       :QuDaoId="QuDaoId" :numID="numID"></LookAtMoreAlter>
      <div>
        <div class="yuyue">
    ...
    watch:{
      ...
      tableListData: function (newLength, oldlength) {
        ...
        if (newLength.length > 0) {
          ...
          // 新增
          let ControllerUrl = `${this.$apidomain}/officialProductLabelController/all?page=1&rows=20&type=1&serviceId=${this.numID}`;
          this.$http.get(ControllerUrl).then(res => {
            console.log(res)
            if (res.data.code === "0000") {

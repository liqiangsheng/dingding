...
//          if (width < 800) {
//            window.location.href= this.$common.siteAppDomain;
//          }
    ...
    this.$http.get(url2).then(r=>{
      ...
      if(data.code==='0000'){
        ...
        let totalChannel = data.result;
        totalChannel.forEach((obj,i)=>{
          obj.childChannel=[];
          this.$http.get(urls+obj.id).then(res=>{
            let datas = res.data;
            if(datas.code==="0000"){
               datas.result.forEach((j,k)=>{
        ...
        this.$store.commit('getMainChannel',totalChannel);
      }
    });

      ...
      submitAdd() {
        ...
        if (this.inputs[0].value) {
          ...
          if (res2.test(this.inputs[0].value) == false) {
            this.inputs[0].isBoolthis.inputs[0].isBool = " 请填写渠道名称，长度不超过20字符，不可输入特殊符号"
            return
          } else {
        ...
        if (!this.inputs[3].value) {
          ...
          let res1 = /^[\u4e00-\u9fa5a-zA-Z0-9]{1,20}$/;

          if (res1.test(this.inputs[3].value) == false) {
            this.inputs[3].isBool = " 请填写联系人姓名，长度不超过20字符，不可输入特殊符号";
            return
          } else {
      ...
      inputs: {
        handler: function (new1, old1) {
          if (new1[0].value != "") {
            let res2 = /^[\u4e00-\u9fa5a-zA-Z0-9]{1,20}$/;
            if (res2.test(this.inputs[0].value) == false) {
              this.inputs[0].isBool = " 请填写渠道名称，长度不超过20字符，不可输入特殊符号"
              return
            } else {
              this.inputs[0].isBool = ""
            }
          ...
          if (new1[1].value != "") {
            ...
            if (res2.test(this.inputs[1].value) == false) {
              this.inputs[1].isBool = " 请填写登录账号，长度不超过20字符，不可输入特殊符号"
              return
            } else {
              this.inputs[1].isBool = ""
            }

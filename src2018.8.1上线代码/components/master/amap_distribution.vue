...
<script>
  ...
  const dd_master_test1=common.masterTest;//请求全部工程师  http://master.test.dingdingkuaixiu.com
  ...
  export default{
      data(){
          return{
              ...
              iframe_src:'',//工程师分布图
//              iframe_src_link:'http://localhost:8080/static/amap_html/amap_profile.html',//给iframe_src做工程师分布图的链接
              iframe_src_link:`${dd_amap_url1}/static/amap_html/amap_profile.html`,//给iframe_src做工程师分布图的链接
          }
      },
      created(){
          let _this=this;
        var dd_querymastergpslist=[]; //请求到的全部工程师的信息，json字符串
        ...
        var dd_teacher_information=function (data,index) {//工程师信息
          console.log('工程师信息',data, index);
          var arr_id=data;
            $.ajax({
              type: "POST",
           ...
           if(res.data.code === "0000"){
             ...
             dd_querymastergpslist_parse.map((arr,index)=>{ //把lon,lat存到sessionStorage里面,把order订单信息存到sessionStorage里面
               ...
               if(arr.masterInfo){
                 ...
                 dd_lat_lon_order.push({masterId:''});//没有就存个空的工程师工号
               }
               dd_lat_lon_order[index]['masterId']=arr.masterInfo.id;//把每个工程师的masterId拿出来存着

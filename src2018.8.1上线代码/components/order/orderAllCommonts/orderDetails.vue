    ...
    <div class="tableList">
      ...
      <div class="productInfo">
        ...
        <table class="tableProduct">
          ...
          <tbody>
           <tr v-for="(item,index) in productList.tableListData" :key="index" :class="{active:item.status == 1}">
             <td colspan="14" v-if="item.isflag" class="onlyClass">
               <ul>
                 <li  v-for="(todo,index) in item.mountings" :key="index">
                  <span> <font style="color: rgba(94, 109, 130, 1);display: inline-block;width: 45px">
                    配件{{index+1}}:</font> &nbsp {{todo.serviceMountingsName}}*{{todo.serviceMountingsCount}} </span>
                   <span>￥{{todo.serviceMountingsPrice}}</span>
                 </li>
               </ul>
             </td>
             <td v-if="!item.isflag">{{index+1}}</td>
             <td v-if="!item.isflag">
             ...
             <td v-if="!item.isflag">{{item.serviceName}}</td>
             <td v-if="!item.isflag">
               <!--'品牌',-->
             ...
             <td v-if="!item.isflag">{{item.serviceSize|placeholder_one}}</td>
             <td v-if="!item.isflag">{{item.costPayableTotal|placeholder_one}}</td>
             <td v-if="!item.isflag">{{item.totalServiceChargeTotal|placeholder_one}}</td>
             <td v-if="!item.isflag">{{item.price1|placeholder_one}}</td>
             <td v-if="!item.isflag">{{item.price4|placeholder_one}}</td>
             <td v-if="!item.isflag">{{item.realCollectionTotal|placeholder_one}}</td>
             <td v-if="!item.isflag">{{item.warrantyDatetime|moment('YYYY-MM-DD HH:mm:ss')}}</td>
             <td v-if="!item.isflag">
               <span
      ...
      <div class="essentialInfo">
        <div class="left">
          ...
          <table :class="{edit:this.isEdit()}">
            ...
            <tr>
              <td>工单渠道</td><td>{{dataObj.officialPartnerSubsetName|placeholder}} </td>
              <td>渠道商订单号</td><td>{{dataObj.thirdOrderId|placeholder}}</td>
              <td>渠道质保</td>
            ...
            <tr>
              <td>预约时间</td><td>{{dataObj.makeTime||placeholder}}</td>
              <td>下单时间</td><td>{{orderData.createTime|moment('YYYY-MM-DD HH:mm:ss')}}</td>
              <td>完成时间</td><td>{{orderData.serviceTimeComplete|moment('YYYY-MM-DD HH:mm:ss')}}</td>
            </tr>
            <tr>
              <td>网点名称</td><td colspan="5">{{orderData.site||"无"}}</td>
            </tr>
            <tr>
              <td>工程师姓名</td><td>{{dataObj.masterName|placeholder}}</td>
              <td>工程师类型</td><td><span v-if="!!masterInfos"> {{masterInfos.type|masterType}}</span> <span v-else>无</span></td>
              <td>工程师工号</td><td>{{dataObj.masterId||placeholder}}</td>
            </tr>
            <tr>
              <td>工程师手机号</td><td>{{dataObj.masterPhoneNum|placeholder}}</td>
            ...
            <tr>
              ...
              <td>预约次数</td><td>{{orderData.appointmentSize}}</td>
              <td >服务时长</td><td>{{orderData.workTimeLimit}}</td>
            </tr>
      ...
      <div class="maintenanceInfo">
        ...
        <table v-if="telescopic5">
          <tr style=" color:rgba(94,109,130,1);">
            <td>序号</td><td>时间</td><td>返修工单号</td><td>备注</td>
          </tr>
      ...
      findOne(){
        ...
        this.$http.get(url,params1).then(r=>{
          ...
          if(data.code==='0000') {
            ...
            this.productList.tableListData.forEach((obj,i) =>{
              this.count = obj.mountings.length;
              Object.assign(obj,{isShow:this.count});
              if(obj.isShow !== 0){
                let  someObj = {};
                for(let k in obj){
                  someObj[k] = obj[k]
                ...
                someObj.isflag = true;
                this.productList.tableListData.splice(i+1,0,someObj);
              }
            });
       ...
       // background:#F4F4F4;
        >.left{
          ...
          .edit{
            ...
            .Deposit{background: #FFFFFF;}
            tr:nth-of-type(3)>td:nth-of-type(6){background: #FFFFFF;}
          }
        }
        >.right{
          width: 34%;

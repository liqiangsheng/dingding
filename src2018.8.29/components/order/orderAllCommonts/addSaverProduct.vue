          ...
          <div class="dable_lsit">
            ...
            <!--导出按钮结束-->
              ...
              <table cellSpacing="0px" cellpadding="0">
                ...
                <tbody>
                <tr v-for="(item,index) in tableList" v-if="item.status==='0'">
      ...
      <ul class="alert_forbidden_text">
        ...
        <li id="product_option">
          ...
          <el-select
            ...
            v-model.trim="addProductInfo.productMsg.SourceTypeValue" placeholder="请选择"
          >
            <el-option
        ...
        <li v-show="label.SourceType.length>0">
          标签：
          <el-select
            v-model.trim="label.SourceTypeValue" placeholder="请选择"
      ...
      this.$http.get(url,params).then(r=>{
        ...
        if(data.code==='0000') {
          data.result.coreMainOrderServices.forEach((v)=>{
            let obj={};
            obj.serviceId=v.id;   //数量
            obj.serviceSize=v.serviceSize;   //数量
    ...
    methods: {
      confirm(isAddProduct,tableList){
        ...
        this.$http.post(url,params).then(r=>{
          ...
          if(data.code==='0000'){
            console.log(data);
            this.$queryFun.successAlert.call(this,'新增服务产品成功',"1");
            if(this.quiry){
              this.quiry();
            }
      ...
      masterImposeOperate(item,isAShow){       //确定
        ...
        let obj={};
        obj.serviceId=item.selectorBehindObj.list.id;   //数量
        obj.serviceSize=item.selectorBehindObj.sum;   //数量
...
//        isAShow.isShow=false;
      ...
      handleChange(value) {
        ...
        let channelId=this.isAddProduct.data.officialPartnerId;
